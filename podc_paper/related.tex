...
a wait-free queue.
Then, Kogan and Petrank \cite{10.1145/2370036.2145835} introduced the 
fast-path slow-path methodology for making data structures wait-free:
...
with their previous wait-free queue (as the slow path).
Ramalhete and Correia \cite{RC17} added a different helping mechanism to the MS-queue.
Although these approaches can improve practical performance,
their amortized step complexity remains $\Omega(p)$. 
...
\Eric{I modified the wording.  I also added the material about fast-path slow-path here (since I had forgotten that paper also applies it to a queue), and fixed the paragraph about FPSP method below when discussin array-based queues.}
...
Other queues \cite{Nik19,10.1145/3490148.3538572,DBLP:conf/ppopp/YangM16} have also used this methodology
