...
\State \tt{<b\sub{enq}, i\sub{enq}>=} \Call{FindResponse}{b\sub{deq}, i\sub{deq}} 
\label{deqRest}
\If{\tt{i\sub{enq}==-1}}
\State \tt{output= null}
\Else
...
\Function{<int, int>}{FindResponse}{\sl{int} b\sub{d}, \sl{int} i\sub{d}}\cmt{Returns \tt{<b\sub{e},i\sub{e}>}, if $E_{root,b_e,i_e}$ is the response to the $D_{root,b_d,i_d}$. Returns \tt{<-1,-->} if the queue is empty.}
...
\Statex
\Statex $\leadsto$ \textsf{Precondition: \tt{root.blocks[end].sum\sub{enq} $\geq$ \tt{e} $\geq$ 1}}
...
\State \tt{start= max(start-(end-start), 0)}
\EndWhile
\State \tt{b= root.BSearch(sum\sub{enq}, e, start, end)}
\State \tt{i= e- root.blocks[b-1].sum\sub{enq}}
\State\Return \tt{<b,i>}
\EndFunction{DSearch}
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[hbt]  
  \center\includegraphics[width=7in]{pics/blockSumEnq.png}
