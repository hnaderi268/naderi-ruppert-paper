...
\begin{figure}[hbt]
  ...
  \caption{The second failed Refresh is assuredly concurrent with a successful \texttt{Refresh}.}
\end{figure}
\it{TODO::simplify the Figure, remove code invocations}
...
\paragraph{Introduce how to create Queue using Get and Index} In our case of implementing a queue, we can make an assumption that one process only wishes to know the rank of a \nf{Dequeue} and one tries to get an \nf{Enqueue} with an specific rank. \nf{Enqueue}s and \nf{Dequeue}s are appended to the tree and when we want to find the response to a \nf{Dequeue}, we compute the place of the \nf{dequeue} in the linearization. Then, using the rank of the \nf{Dequeue} among \nf{Deqeueue}s and the information about the size of the queue stored in the root we compute which \nf{Enqueue} is the answer to the \nf{Dequeue} or if the answer is null. 



\paragraph{}
A non-null \nf{Dequeue} is one that returns a non-null value.
If the queue is non-empty and $r$ \texttt{Dequeue} operations have returned a non-null response, then $i$th \texttt{Dequeue} returns the input of the $r+1$th \texttt{Enqueue}. So in order to answer a \nf{Dequeue}, it iss sufficient to know whether the queue is empty or not and the number of previous non-null \nf{dequeue}s.

\paragraph{}
We know we can linearize operations in a block in any order; here, we choose to decide to put \nf{Enqueue} operations in a block before \nf{Dequeue} operations. In the next example, operations in a cell are concurrent. \texttt{Dequeue()} operations return \texttt{null, 5, 2, 1, 3, 4, null} respectively. We will next describe how these values can be computed efficiently.
...
\paragraph{Block}
Block $b$ contains subblocks in the left and right children. The left subblocks of $b$ are some consecutive blocks in the left child starting from where the block prior to $b$ ended. See Figure \ref{fig::createBlock} .

We store ordering among \texttt{operation}s in the tournament tree constructed by \texttt{node}s. In each \texttt{node} we store pointers to its parent and children, an array of \texttt{block}s and the index \nf{head} of the first empty entry in \texttt{blocks}. Furthermore, in \texttt{leaf} nodes there is an array of \texttt{operations} where each \texttt{operation} is stored in one cell with the same index in \texttt{blocks}. There is a \texttt{counter} in each \texttt{node} incrementing after a successful \texttt{Refresh} step. It means after that some bunch of \texttt{block}s in a node have propagated into the parent then the \texttt{counter} increases. Each new \texttt{block} added to a node sets its \texttt{time} regarding \texttt{counter}. This helps us to know which blocks have aggregated together to a block, not precisely though. We also store the index of the aggregated \texttt{block} of a \texttt{block} with \texttt{time} $i$ in \texttt{super[i]}. 
...
\it{I believe the code is readable and we do not need references to code.}
