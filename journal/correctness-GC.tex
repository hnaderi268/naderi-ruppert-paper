...
The \op{AddBlock} that creates $T'$ may optionally \op{Split} the RBT \rev{at line \ref{splitGC}}
and then add $B$ to it \rev{at line \ref{GCend} or \ref{noGC-insert}}.
So the claim is satisfied.
\end{proof}
...
Then some other \op{CAS} on $\var{v}.\fld{blocks}$ succeeds between \rev{each \op{Refresh}'s 
...
then \rev{the \dequeue} has been propagated to the root and its result has been written in the \fld{response} field of the leaf block it created.
...
Let \rev{$s$ be the step} when $A$ invokes \op{SplitBlock} on line \ref{callSplitBlock}.
