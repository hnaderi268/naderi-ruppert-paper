...
if one operation $op_1$ terminates before another operation $op_2$ begins, then $op_2$ does not precede $op_1$ in $L$.  We also show each completed \dequeue\ returns the same result \rev{in the concurrent execution} as it would in the sequential execution~$L$ \rev{(i.e., the sequential execution of the operations in the order given by $L$)}.
