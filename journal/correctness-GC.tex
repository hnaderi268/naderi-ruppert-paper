...
\here{need to prove a lemma analogous to Lemma \ref{blockSize}}
...
Since each block is non-empty by Corollary \ref{blockNotEmpty}, $T'$ contains at most $2q_{max}+4p$ blocks, and one more 
...
We argue that each process $P$ can do an $r$th helping phase on \var{v} at most once.
...
will have $\fld{index}>r\cdot G$, so $P$ will not perform an $r$th helping phase on \var{v} again.

Each helping phase takes $O(p)$ steps to read the \var{last} array, and
...
Since each operation still  does only two \op{CAS} instructions at each level of the tree (at line \ref{casGC}), the following proposition still holds for the space-bounded version of the queue.
