...
\begin{lemma}
If the maximum \fld{index} in a node's RBT is a multiple of $p^2$, then
...
and call \op{GetEnqueue}, which does $O(1)$ RBT operations (including a search at line \ref{getChildGC}) and other work at each level of the tree.
...
For each integer $r$, each process does GC on each node $v$ with a block whose \fld{index} is $rp^2$ at most
once, since either its CAS succeeds, or another processes CAS advances the maximum 
Each such helping phase takes $O(p)$ time to read the \var{last} array,
$O(p \log p \log(p+q_{max}))$ time to \op{Help} (excluding any GC phases within \op{Help}, which will be accounted for at the other nodes),
and \here{to be continued}
\end{proof}
