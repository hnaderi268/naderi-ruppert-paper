...
\paragraph{Introduce Double Refresh} To do a propagate step on node $n$ in the tree, a process observes the operations in both of $n$'s children that are not already in $n$, merges them to create an ordering and then tries to append the ordering to the sequence stored in $n$. We call this procedure \tt{$n$.Refresh()}. A process doing a \nf{Refresh} on $n$ with successful append helps other processes doing \nf{Refresh} on $n$ concurrently to propagate their operations up to the parent. The key idea is that if a process invokes \tt{Refresh} on the node $n$ two times and fails to append the new operations to $n$ both times, the operations that were in $n$'s children before the first \tt{Refresh} are guaranteed to be in the node after the second failed \tt{Refresh}.
This is because if both of the \tt{Refresh}es on $n$ fail to append then there is another instance of \nf{Refresh} in between which has gathered its new operations after the first failed \nf{Refresh} and succeeded to do an append. This \nf{Refresh} appends the operations that the first failed \nf{Refresh} was trying to append.
...
\end{subfigure}
\hspace{6em}
\caption{\label{fig::propagstep} Before and after of a \nf{$n$.Refresh} with successful append. Operations propagating from the left child are numbered with $l$ and from the right child by $r$.}
\end{center}
\end{figure}

\begin{figure}[hbpt]
  \center\includegraphics[width=4in]{pics/doublyrefresh.drawio.png}
  \caption{The second failed Refreshof $P$ is assuredly concurrent with a successful \texttt{Refresh} that has read its operations after the first failed \nf{Refresh}.}
\end{figure}

\paragraph{Introduce CAS} We use \nf{CAS} (Compare \& Swap) instructions to implement the \nf{Refresh}'s Try to Append mechanism in described in the previous paragraph. After a process appends its operation into its leaf it can call \nf{Refresh} on the path up to root two times on each node. So with $O(\log p)$ \nf{CAS}es per operations we can have a tree agreeing on the linearization. This cooperative solution allows us to overcome the CAS Retry Problem.


\paragraph{Introduce Blocks} It is not efficient to store the sequence of operations in each node explicitly because each operation would have to be copied all the way up to the root; doing this would not be poly-logarithmic time.
Instead we use implicit representation of the operations propagated together. Furthermore we do not need to maintain an ordering on operations propagated together in a node until they have reached the root. We can only keep track of set of operations in each \nf{Refresh} and then define the linearization only in root (see Figure \ref{fig::set}). Achieving a constant sized implicit representation of operations in a \nf{Refresh} allows us to \texttt{CAS} fixed-size objects in each \tt{Refresh}. To do that, we introduce \it{block}s. A block is a piece of information about the operations in a \nf{Refresh} step. It contains the number of operations from the left and the right child in a \texttt{Refresh} procedure (see Figure \ref{fig:block} for an example). A node stores an array of blocks of operations propagated up to it. A propagate step  aggregates the new blocks in children into a new block and puts it in the parents' blocks.  In each \nf{Refresh} there is at most one operation from each process trying to be propagated, because one operation cannot invoke two operations concurrently which means there are less than or equal $p$ operations in a block. Furthermore, since the operations in a \texttt{Refresh} step are concurrent we can linearize them  among themselves in any order we wish, because if two operations are read in one successful \tt{Refresh} step in a node they are going to be propagated up to the root together. Our choice is to put the operations propagated from the left child before the operations propagated from the right child. In this way if we know the number of operations from the left child and the number of operations from the right child in a block  we have a complete ordering on the operations.
...
\end{tikzpicture}
\caption{\label{fig::set} Leaves are for processes $P_1$ to $P_4$ from left to right. In each internal node and one can arbitrarily linearize the sets of concurrent operations  propagated together in a \nf{Refresh} among themselves. For example $op_4^1$ and $op_3^2$ have propagated  together in one \nf{Propagate} step and they will be propagated up to the root together. Since their execution time window have range in common they can be linearized in any order between themselves.}
\end{center}
\end{figure}
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:block} Showing concurrent operation sets with blocks. Blocks between two lines $||$ are propagated together to the parent. Each block consists of a pair(left, right) indicating the number of operations from the left and the right child, respectively. For example, block (12,6) in the root contains blocks (10,2) from the left child and (6,0) from the right child. The third block in the root (8,36) is created by merging block (5,3) from the left child and (14,6) and (0,16) from the right child. Block (5,3) is superblock of blocks (0,5) and (1,2) and blocks (5,1),(3,5) and (4,2) are subblocks of (14,6).}
\end{figure}

\paragraph{Introduce Get and Index} So far, we have a shared tree that processes use to agree on the implicit ordering stored in its root. With this agreement on linearization ordering we can design a universal construction; for given object $O$ and operation $op$ by applying all the operations till $op$ in the root on a sequential instance of the object and then return the response. Having this agreement by itself is not enough for an efficient queue.
A process may wish to know (1) the $i$th propagated operation or (2) the rank of a propagated operation in the linearization. We will explain how to use (1) and (2) to construct a fast queue in the next paragraph.
...
To get the $i$th operation in the root, we find the block $B$ containing $i$th operation in the root, and then recursively find the subblock of $B$ that contains $i$th element. To make this recursive search faster, instead of iterating over all blocks in the node, we store the prefix sum of the number of elements in the blocks sequence to permit a binary search. We also store pointers to determine the range of subblocks of a block to make binary search faster. In each block, we store the prefix sum of operations from the left child and the right child. Moreover, for each block, we store two pointers to the last left and right subblock of it (see Figure \ref{fig::pointer}). We know a block size is at most $p$, so binary search takes at most \textsc{O}$(\log p)$ time, since the pointers of a block and its previous block reduce the search range size to \textsc{O}$(p)$.
To compute the rank in the root of an operation in the leaf, we need to find the superblock of the block that operation is in. After a block is installed we set the approximate index of the superblock of the block in it.
...
\paragraph{Introduce how to create Queue using Get and Index}

In an execution on a queue where no dequeue operation returns null, the $k$th dequeue returns the element of the $k$th enqueue. In general case a dequeue returns null if and only if the size of the queue after the previous operation is 0. Otherwise if the the dequeue is the $k$th non null returning dequeue it returns the element of $k$th enqueue. Having the size of the  queue after an operation we can compute the number of non null returning dequeues  from the number of enqueues up to block B and size of the queue after B. So if we store the size of the queue after each block operations in the root we can compute the index of the enqueue which is response to a dequeue in constant time.

In our case of implementing a queue, we can make an assumption that one process only wishes to know the rank of a \nf{Dequeue} and one tries to get an \nf{Enqueue} with an specific rank.  We know we can linearize operations in a block in any order; here, we choose to decide to put \nf{Enqueue} operations in a block before \nf{Dequeue} operations. In the next example, operations in a cell are concurrent. \texttt{Dequeue()} operations return \texttt{null, 5, 2, 1, 3, 4, null} respectively.
...
\end{small}
  ...
  \caption{An example history of operation blocks on the queue. \nf{Enqueue}s are shown with \nf{ENQ} and \nf{Dequeue}s are shown with \nf{DEQ}.}
\end{table}


\paragraph{}
Now, we claimed that by knowing the size of the queue, we can compute the rank of the resulting \texttt{Enqueue()}. We apply this approach to blocks; if we store the size of the queue after each block of operations happens, we can compute each \nf{Dequeue}'s index of result in \textsc{O}$(1)$ steps.
...
\begin{small}
  \begin{tabular}{c|c|c|c|c}
    \hline &\texttt{DEQ} & \texttt{ENQ(5)}, \texttt{ENQ(2)}, \texttt{ENQ(1)}, \texttt{DEQ}& \texttt{ENQ(3)}, \texttt{DEQ}&  \texttt{ENQ(4)}, \texttt{DEQ}, \texttt{DEQ}, \texttt{DEQ}, \texttt{DEQ}\\ \hline
    \tt{\#ENQs} & 0 & 3 & 1 & 1 \\ \hline
    \tt{\#DEQs} & 1 & 1 & 1 & 4 \\ \hline
    \tt{size} & 0 & 2 & 2 & 0 \\ \hline
  \end{tabular}
\end{small}
  \caption{Augmented history of operation blocks on the queue}
\end{table}

\it{Do you think the remainig part is overkill? I think it is to much detail which is also repeated in the proof.}
...
Given \texttt{DEQ} is in block $B$, response of \nf{DEQ}  is the \tt{Enqueue} with index \nf{non-null Dequeus[b-1]+ index of DEQ in the block's Dequeus} in the root, if \texttt{(size[b-1]- index of DEQ in the $B$'s Dequeus >=0)}. Otherwise it would be \nf{null}


\subsection{Details of Implementation}

\paragraph{Node} (\it{Repetitive as the fields description?})
 In each \texttt{node} we store pointers to its parent and children, an array of \texttt{block}s and the index \nf{head} of the first empty entry in \texttt{blocks}.

\paragraph{Block} (\it{Repetitive as the fields description?})
Block $b$ contains subblocks in the left and right children. The left subblocks of $b$ are some consecutive blocks in the left child starting from where the block prior to $b$ ended. In each \texttt{block} we store 4 essential stats that implicitly summarize which operations are in the block \texttt{sum\textsubscript{enq-left}}, \texttt{sum\textsubscript{deq-left}}, \texttt{sum\textsubscript{enq-right}}, \texttt{sum\textsubscript{deq-right}}. \nf{end\sub{left}} and \nf{end\sub{right}} field show the last subblock of a block in left and right child.  \texttt{size} in a \texttt{block} in \nf{root} stores the size of the queue after the \texttt{block} has finished. 

\paragraph{Enqueue(e)}
An \nf{Enqueue} operation does not return a response, so to do an \nf{enqueue} operation in our algorithm it is sufficient to just append the \nf{Enqueue} operation to the root, then use its position in the linearization for future \nf{Dequeue} operations. \nf{Enqueue($e$)} creates a \nf{LeafBlock} with $\nf{element}=e$, sets its \nf{sum\sub{enq}} and \nf{sum\sub{deq}} fields and then appends it to the tree.

\it{I believe the code is readable enough and we do not need references to code.}

\paragraph{Dequeue()}
\nf{Dequeue($e$)} creates a \nf{LeafBlock}, sets its \nf{sum\sub{enq}} and \nf{sum\sub{deq}} fields, appends it to the tree. Then computes the position of the appended \nf{dequeue} operation in the root and after that finds the response of the \nf{dequeue} calling \nf{FindResponse}.

\paragraph{FindResponse(b,i)}
To compute the response of the $i$th \nf{dequeue} in $b$th block of the root Line \ref{checkEmpty} computes whether the queue is empty or not. If there are more dequeues than enqueues the queue would become empty before the requeusted dequeue. If the queue was not empty, Line \ref{computeE} computes the position of the eqneueue which is response to the dqeueue. Knowing the response is the $e$th enqueue in the root which is before the $b$th block we find the block and position contaiing the qnueue operation using \nf{DSearch} and after that \nf{GetEnqueue} finds its \nf{element}.

\paragraph{Append(blk)}
\nf{head} field is the index of the first empty index in \nf{blocks} array in a \nf{LeafBlock}. \nf{Append($B$)} adds $B$ to the end of the \nf{blocks} in the leaf, increments \nf{head} and then calls \nf{Propagate} on the leaf's \nf{parent}. When \nf{Propagate} terminates it is guaranteed that the appended block is subblock of a block in the \nf{root}. There are no multiple write accesses on \nf{head} and \nf{blocks} in a leaf because only the process that the leaf belongs to appends to it. 

\paragraph{n.Propagate()}
\nf{Propagate($n$)} uses the double refresh idea and invokes two \nf{Refresh}es on $n$ in Lines \ref{firstRefresh} and \ref{secondRefresh}. Then invokes the \nf{Propagate} on \nf{$n$.parent} recursivly till it reaches the root. 

\paragraph{n.Refresh()}
\nf{$n$.Refresh} goal is to  create a block of $n$'s children new blocks and append it to $n$\nf{.blocks}. Lines \ref{startHelpChild1}-\ref{endHelpChild1} of \nf{$n$.Refresh} help to \nf{Advance} $n$'s children. \nf{Advance} increments the children's \nf{head} and set the \nf{super} field of the last block in $n$. The reason behind helping \nf{super} field is explained later in \nf{IndexDequeue} paragraph. After helping to \nf{Advance} children, the value $h$ from $n$\nf{.head} is read at Line \ref{readHead} and \nf{new} block is created in Line \ref{invokeCreateBlock}. \nf{new} is going to be inserted into $n\nf{.block[h]}$. The reason why reading \nf{head} and \nf{CreateBlock} are in two lines is that maybe the \nf{head} value changes from reading it till creating a block to be inserted to \nf{head}. Then if \nf{new} is empty \nf{Refresh} returns \nf{true} because there is no new operations to propagate  an it is unnecessary to add an empty block to the tree. Later we will assume all blocks contain at least one operation. Line \ref{cas} tries to install \nf{new}, if it was succsuful all is good, if not it means someone else has put a block earlier into \nf{head} or maybe the \nf{head} was not empty at first point. Anyway \nf{Refresh} tries to update the \nf{head} and \nf{super} field of $n$\nf{.blocks[h]} at Line \ref{advance}.


\paragraph{CreateBlock} \texttt{$n$.CreateBlock(h)} returns a block containing new operations of \texttt{n}'s children.
The \nf{new} block created in Line 333's fields are filled with symmtery for both left and right directions. \nf{index\sub{last}} is the index of the last subblock in the direction and \nf{index\sub{prev}} is the index of the prvious block of the first direct subblock to be aggregated into \nf{new}. With this said, \texttt{$n$.blocks[$i$\textsuperscript{$\prime$}].end\textsubscript{dir}} stores the index of the rightmost direct subblock of direction \nf{dir} child of \texttt{$n$.blocks[$i-1$]}. Then \nf{sum\sub{enq-dir}} is computed with the sum of the  from computing the number of enqueue operations in the \nf{new} block and the value stored in \texttt{$n$.blocks[$i-1$].sum\sub{enq-dir}}. \nf{sum\sub{deq-dir}} is also the same. Then if \nf{new} block is going to be installed in the \nf{root}, the \nf{size} field is computed.

\it{Should I get into details of computing size and sumenq?}
...
\paragraph{DSearch(e,end)}
We can show an operation in a node in two formats, the rank of the operation among all the operations in the node or the index of the block containing the operation in the node and the rank of the operation in that block. If we know tuple of block and rank in the block of an operation ($E$) we can find the this tuple in the subblock containig the operation in poly log time. To find the response of a dequeue, the information we know about the response enquue is like the first way of showing the operation in the root ($E_i(\nf{root})$). So we have to find the block in the root containing the \nf{e}th enqueue in the root. We also know \nf{end} which the \nf{e}th enqueu is before or in the \nf{root.blocks[end]}. \nf{DSearch} uses the Doubling idea to find the range that the answer block is there (Lines \ref{dsearchStart}-\ref{dsearchEnd}) and then tries to find the response with binary search (Line \ref{dsearchBinarySearch}).

\paragraph{GetEnqueue(b,i)}
\nf{$n$.GetEnqueue(b,i)} returns the \nf{element} of \nf{i}th enqueue in $b$ th block of $n$. The range of subblocks of a block can be determined with \nf{end\sub{dir}} fields of the block and its previous block. Then the subblock can be found usig binary search on \nf{sum\sub{enq}} field.

\paragraph{IndexDequeue(b,i)}
...
\Function{Object}{Dequeue()}{} \cmt{Creates a block with null value element, appends it to the tree and returns its response.}
\State \tt{block newBlock= \Call{new}{\sl{LeafBlock}}} 
...
\Function{<int, int>}{FindResponse}{\sl{int} b, \sl{int} i}\cmt{Returns the response to $D_{root,b,i}$.}
...
\cmt{Returns \tt{<b,i>} such that $E_e(root)=E_i(root,b)$.}
...
\Function{void}{Append}{\sl{block} blk} \cmt{Only called by the owner of the leaf.}
...
\pagebreak
\Statex $\leadsto$ \textsf{Precondition: \tt{blocks[start..end]} contains a block with field \tt{f} greater than or equal to \tt{i}}
\Statex $\triangleright$ \textmd{Does a binary search for~the value \tt{i} of the given prefix sum \tt{field}. Returns the index of the leftmost block in \tt{blocks[start..end]} whose \sl{field} \tt{f} is $\geq$ \tt{i}}.
\Function{int}{BinarySearch}{\sl{field} f, \sl{int} i, \sl{int} start, \sl{int} end}
\State \Return \tt{min\{j: blocks[j].f$\geq$i\}}
...
\State \tt{new.end\sub{dir}= index\sub{last}} \label{endDefLine}
...
If a \nf{Dequeue} operation returns null it is called a \it{null} \nf{Dequeue}, otherwise it is called \it{non-null} \nf{Dequeue}.
\end{definition}
...
\paragraph{Infinite Sized Arrays}
Our queue algorithm's usage of memory has two places to improve. First, we assumed arrays in the nodes are infinite sized. Second we do not handle garbage collection, dequeued operations are still remain in the nodes. We can use list to arrays to resolve the infinite sized arrays issue. To implement an infinite shared array we can create a small array of pointers called \nf{arr}. When a process wishes to write into \nf{head} it checks whether \nf{arr[$\lfloor\log $ head$\rfloor]$]} points to an array or not. If not it creates a shared array with size $2^i$ and tries to \nf{CAS} it to the created array. After \nf{CAS} terminates whether it is successful or not \nf{arr[$\lfloor\log $ head$\rfloor]$]} points to an array. When a process wishes to access to $i$th element it looks upto \nf{arr[$\lfloor\log i\rfloor]$][$i-2^{\lfloor\log i\rfloor]}$]} Note that CAS Retry Problem does not happen here because if $n$ elements are appended to the array then only $O(p\times\log n)$ times \nf{CAS} has happened. And if the size of the all shared memory is $k$, \nf{arr} can be initialized to a $\log k$ sized array.

\begin{figure}[hbt]  
  \center\includegraphics[width=6.5in]{pics/doublingArray.png}
  \caption{Infinite sized Arrays}
  \label{fig::doublingArray}
\end{figure}

\paragraph{Garbage Collection}
To garbage collect we can keep track of the index of the most recent \nf{block} in the \nf{root} that has been term and then split the blocks from that \nf{block}. We can use Persistent Red Black Trees because they support split in an efficient method.

\paragraph{Poly-logarithmic Wait-free Vectors}
If we do not distinct between \nf{Enqueue}s and \nf{Dequeue}s and replace \nf{DSearch} with Binary Search on all the \nf{root} in our algorithm, we achieve a list with poly-logarithmic \nf{Append}, \nf{Get} and \nf{Index} methods with respect to the number of \nf{Append}s done.

\paragraph{Stacks or maybe other objects that store sequences}
The reason behind why our \nf{tree} worked well to implement a queue was two things. First, we can compute the index of the \nf{Enqueue} that is response of a \nf{Dequeue} fast. Two, the response of the \nf{Dequeue} is not very far from it (size of the queue). We think we can create wait-free poly-logarithmic version of other objects which their operations satisfy these two conditions. 
