...
\State \tt{i+= blocks[sb].num\sub{deq-left}}  \label{considerLeftBeforeRight}
...
The next two observations are correct for both enqueues and deqeueues.

\begin{observation}
  $n\nf{.blocks[}b\nf{]}\nf{.num\sub{enq}}$ is the size of $E(n,b)$.
\end{observation}


\begin{observation}\label{lem::sumDeq}
$\sum_{i=1}^{b} {E(n,i)}=n\nf{.blocks[}i\nf{].sum\sub{enq}}$.
\end{observation}



The next observation is correct for both directions.

\begin{observation}\label{lem::numEnqLeft}
Let $B$ be $n\nf{.blocks[}b\nf{]}$. $B\nf{.num\sub{enq-left}}$ is the number of \nf{Enqueue} operations in the direct subblocks of $B$ in $n$\nf{.left}.
\end{observation}
\begin{proof}
Let $B^\prime$ be $n\nf{.blocks[}b-1\nf{]}$. We prove the claim by induction on height of node $n$. 
\begin{align*}
    B\nf{.num\sub{enq-left}}=&B.\nf{sum\sub{enq-left}}-B^\prime.\nf{sum\sub{enq-left}}& \text{Definition of }\nf{num\sub{x}}\\
    =&B^\prime.\nf{sum\sub{enq-left}}+n\nf{.left.blocks[}B\nf{.end\sub{left}].sum\sub{enq}}\\&-n\nf{.left.blocks[}B^\prime\nf{.end\sub{left}].sum\sub{enq}}-B^\prime.\nf{sum\sub{enq-left}} & \nf{CreateBlock} \text{ code}\\
    =&\sum_{i=B^\prime\nf{.end\sub{left}}+1}^{B\nf{.end\sub{left}}}  n\nf{.left.blocks[}i\nf{].num\sub{enq}}
\end{align*}
\end{proof}
\pagebreak
...
\begin{proof}
Let the first successful \nf{$n$.parent.Refresh} after $R_n$ be $R_{p1}$ and the second next successful \nf{$n$.parent.Refresh} be $R_{p2}$. To derive a contradiction assume $B$ was not propagated to $n$\nf{.parent} neither by $R_{p1}$ nor by $R_{p2}$.
...
$R_n$ puts $B$ into $n$\nf{.blocks[}$b$\nf{]} so $n.$\nf{head} is $b$ at $_{\ref{cas}}^{R_n}t$. Since $R_{p2}$'s \nf{CAS} into $n$\nf{.blocks} is successful there should be a \nf{Refresh} instance $R_p^\prime$ on $n$\nf{.parent} that increments $n$\nf{.parent} after $R_{p1}$'s \ref{cas}. $R_{p2}$'s \ref{readHead} is after $R_{p2}$'s \ref{incrementHead}. $R_p^\prime$'s \ref{incrementHead} is after $R_{p1}$'s \nf{CAS}. We assumed $t^{R_n}_{\ref{cas}}<t^{R_{p1}}_{\ref{cas}}<t^{R_{p2}}_{\ref{cas}}$. Since \ref{readChildHead} is after \ref{readHead} inc the code, $R_{p2}$ read a value $\geq b$ for $n.\nf{head}$.
...
Furthermore, if $B$\nf{.super} is not setset earlier it is set by $R_{p2}$ in Line \ref{setSuper1} invoked from \ref{helpAdvance}.
...
\end{corollary}
\begin{proof}
\nf{super} field of a block is set after installing it  to the value of the parent node's \nf{head} by \nf{Advance}. From previous Lemma we know that the block is propagated by second next successful \nf{Refresh} on parent, so the value written in $b$.\nf{super} can  the \nf{super} field of a lemma i
\end{proof}
TODO: complete proof
