...
and divide the number of steps in the execution by the number of operations initiated in the execution.)
...
whose step complexity is sublinear in $p$.
Our queue is the first with step complexity that is polylogarithmic in $p$ and in $q$, the number of elements in the queue.
...
while still having amortized step complexity $O(\log^2p + \log q)$ per operation.
Both versions use single-word CAS on reasonably-sized words.
