...
Let the first successful \nf{p.Refresh()} be $R_{p1}$ and the second next successful \nf{p.Refresh()} be $R_{p2}$. If $R_{p1}$ reads \nf{x} in Line \ref{readHead}, then $R_{p2}$ has to read \nf{x+1} in Line \ref{readHead} (induced from \ref{lem::headProgress}, \ref{nonDecreasingHead}). See the timeline in Figure \ref{fig::timeLine2} for two consecutive successful \nf{Refresh()} instances $R_{p1}$,$R_{p2}$  on ${p}$.
