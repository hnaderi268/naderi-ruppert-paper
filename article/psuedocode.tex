...
\EndFor
\State \tt{\Call{CAS}{size, h, h+1}} \label{incrementHead1}
\State \Return{\tt{true}}
\Else
\State \tt{\Call{CAS}{size, h, h+1}} \cmt{Even if another process wins, help to increase the size. The winner might have fallen sleep before increasing \tt{size}.} \label{incrementHead2}
...
\framebox[1.1\width]{TEST} As a temporary test I have changed the name of n.size to n.established here, other options are n.head and n.lastBLock but they might be confusing since we have used them before. Fix the logical order of definitions (cyclic refrences).
...
Block \texttt{b} is a subblock of \texttt{n.blocks[i]} if it is $\in$ \texttt{n.left.blocks[n.blocks[i-1].end\textsubscript{left}+1..n.blocks[i].end\textsubscript{left}]} $\cup$ \texttt{n.right.blocks[n.blocks[i-1].end\textsubscript{right}+1..n.blocks[i].end\textsubscript{right}]} or  is a subblock of a block in it.
\end{definition}
...
  If \nf{b} is the block returned by \nf{n.CreateBlock(h, x)} invoked at time $t$, then \nf{ops(EST\sub{n.left, t})} $\cup$ \nf{ops(EST\sub{n.right, t})} - \nf{ops(EST\sub{n, t})} $\subseteq$ \nf{ops(b)}.
\end{lemma}
\begin{proof}
 WLOG we prove the claim for the left child. Blocks in \nf{n.left.blocks[n.blocks[i-1].end\textsubscript{left}+1..n.blocks[i].end\textsubscript{left}]} are all the new established operations at time $t$ by definition of Subblock. Since the head is only increasing (Lemma~\ref{lem::sizeProgress}) the  claim holds after $t$. See Figure~\ref{fig::createBlock}. The right child is the same.
\end{proof}
...
Each block has at most $p$ subblocks.
\end{lemma}
%\begin{proof}
...
\begin{itemize}
\item We call operations before $op$ in the sequence of operations $S$, prefix of the $op$.
...
\begin{lemma}[Get corretness] \label{get}
...
\begin{lemma}[Index correctness]
 \nf{n.Index(b,i)} returns the rank in the $D(root)$ of ith Dequeue in $D(n,b)$.
\end{lemma}
%\begin{proof}
...
\begin{lemma}[Computing Queue's Head] \label{computeHead}
  Let S be the state of an empty queue if the operations in prefix of $i$th dequeue in $D(root,b)$ are applied on it. \nf{FindResponse()} returns the index of the enqueue that is the head in S. If the  queue is empty in S it returns \nf{<-1,-->}.
  \end{lemma}

\begin{lemma}[Validity of \nf{head}]\label{head}
If block b is written in \nf{n.blocks[i]} then \nf{n.blocks[i]} is going to remain b (not overwritten in future).
\end{lemma}
%\begin{proof}
