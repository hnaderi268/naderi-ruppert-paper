...
As we said in  each node the sequence of operations is stored. We implement the sequence using an array and appending to the sequence by doing \tt{CAS} operations on the first null element in the array. In each propagate step, our algorithm uses a subroutine \textsc{Refresh}($n$) that aggregates new operations from node $n$'s both children (that have not already been propagated to $n$) and tries to append them into $n$. The general idea is that if we call \textsc{Refresh}($n$) twice, the operations in $n$'s children before the first \textsc{Refresh}($n$) are guaranteed to be in $n$.
Because if both of the \nf{Refresh()}es fail there is another instance of \nf{Refresh()} in between which has succeeded to do \nf{CAS} and has already appended the operations the first \nf{Refresh} was trying to append. This mechanism makes us to do \nf{TryAppend}s twice instead of \nf{Append}ing to the array. From the next paragraph we explain our ideas which are different from Jayanti.
...
%\end{proof}
 ...
 Let \nf{h} be the value read  from \nf{n.head} when a successful instance of \nf{Refresh()} is called on the node \nf{n}, the instance is supposed to do a \nf{CAS(null, new)} into \nf{n.blocks[h]}. We call the appended block \nf{b}. Despite other fields of a block that are set while creating the block, because we do not know the exact index of the superblock of the block we  can not set the \nf{super} field of a block while appending it to the blocklist. One approach is to set the super of a block when it is porpagated but this is not efficent because there might be $p$ blocks propagating together in a \nf{Refresh()} step. However we know that once a block is installed its stuperblock is going to be close to \nf{n.parent.head} at the time of installation. \nf{super} field is supposed to show the approximate index of the superblock of a block, so we want \nf{super} to be as close to the index of the superblock of the block in the parent. We set the \nf{super} value to \nf{n.parent.head} right after appending the block (Line \ref{}), since \nf{b}'s superblock comes after that in near future. Maybe the process goes to sleep at sometime after appending \nf{b} to \nf{n.blocks} and before setting the super. In this case we use help from the next \nf{Refresh}es on \nf{n} and \nf{n.parent}.
...
\begin{definition}  A Refresh is successful if it reaches Line \ref{cas} and returns true. If \nf{Refresh ()} instance \nf{R1} does its \nf{CAS} at Line \ref{cas} sooner than \nf{Refresh} instance \nf{R2} we say \nf{R1} has happened before \nf{R2}. We knew that \nf{CAS} objects are linearizable.
\end{definition}
%
...
Block \nf{b} is appended to \nf{n.blocks[h]} on Line \ref{cas}. After appending \nf{b}, \nf{super} field of \nf{b} is set on Line \ref{setSuper1} by the same process or another \nf{n.Refresh()} or  maybe an \nf{n.parent.Refresh()}. We want to argue how far \nf{b.super} is from the index of \nf{b}'s super block, which is created by a successful \nf{n.parent.Refresh()} that propagates \nf{b}.

WLOG for the rest of this section assume \nf{n} is the left child of \nf{n.parent}. The first successful \nf{n.parent.Refresh()} that read some value
greater than \nf{h} for \nf{left.head} is going to contain \nf{b} in its created block in Line \ref{invokeCreateBlock}. We call \nf{n.Refresh()} that added \nf{b} the \nf{R\sub{n}} and \nf{n.parent.Refresh()} that propagated \nf{b} to \nf{p}  the \nf{R\sub{p}} and we use \nf{p} instead of \nf{n.parent}. We also sometimes use \nf{n[h]} instead of \nf{n.blocks[h]}(maybe a good idea for the whole document?).

\begin{lemma}
After n blocks[h].CAS(null,b) succeeds an head cannot increase from h to h+1 unless be super is set.
\end{lemma}
\begin{proof}
If an instance of n.Refresh() increments n.head on Line \ref{324}, then n.blocks[head].super was set at Line \ref{323}.
\end{proof}

\begin{corollary}
  If n.blocks[i].super is null, then n.blocks[i+1] is null.
\end{corollary}

As per previous corollary, b.super has to be set before the next successful Refresh() on n after the Refresh() that put b into n. Because if b.super=null then the n head cannot advance and then the next n.Refresh() is going to fail. Now Let us talk about how the n.parent.Refresh()es that took place after the putting b into n, will help to set b.super.

\begin{lemma}
  If b$\in$n.parent.blocks[i] then b.super$\leq$i.
\end{lemma}
\begin{proof}
  I did not quite understand the way Eric told me to prove this.
\end{proof}

\begin{lemma}
If Rn=n.Refresh() puts b in n[h] at $t_{321}^{Rn}$, then the block created by the next successful P.Refresh() or the second next successful p.Refresh() contains b and b.super is set before the second next successful p.Refresh() after $t_{321}^{Rn}$.
\end{lemma}

\begin{proof}
b.super could be set by helping of some n.Refresh() instance before the second next successful p.Refresh(), but even if no one helped it we prove the second next  successful p.Refresh() will help n to advance in Line \ref{setSuper} called by Line \ref{helpAdvance}.

\begin{figure}[hbt]  
  \center\includegraphics[width=7in]{pics/timeLineSuper1.png}
  \caption{b.super is set by some instance of Refresh() on n or n.parent showed by \nf{R'} and \nf{n.head} is incremented by some \nf{Refresh()} called \nf{R"}.}
  \label{fig::timeLine1}
\end{figure}

\nf{Rp} is the first successful \nf{p.Refresh()} instance that has propagated \nf{b} into \nf{p.blocks[x]}. We want to know how great $\nf{x}-\nf{b.super}$ can be? \nf{p.head} is \nf{hp} when \nf{R'} reads it. From Lemma \ref{lem::headProgress} \nf{p.head} could only increase from \nf{hp} to \nf{hp+1} if $\nf{p[hp]}\neq\nf{null}$. In other words there should be a successful \nf{p.Refresh()} in order to \nf{p.head} advance. We claim there cannot be more than one \nf{p.Refresh()} after \nf{R'} reads \nf{p.head} and before $t_{\ref{lastLine}}^{Rp}$.
...
Let the first successful \nf{p.Refresh()} be \nf{Rp1} and the second next successful \nf{p.Refresh()} be \nf{Rp2}. If \nf{Rp1} reads \nf{i} in Line \ref{readHead}, then \nf{Rp2} has to read \nf{i+1} in Line \ref{readHead}. See the timeline in Figure \ref{fig::timeLine2} for two consecutive successful \nf{Refresh()} instances \nf{Rp1, Rp2} on \nf{p}.
