...
In Chapter 3 we introduce a poly-logarithmic step wait-free queue. In Section 3.1 we give a high-level description of our implementation. Section 3.2 contains the algorithm itself.
...
Milman et al.~\cite{DBLP:conf/spaa/MilmanKLLP18} designed a lock-free queue with batching. Their idea of batching allows a sequence of operations to be submitted as a batch for later execution on the MS-queue. They use some properties of the size of the queue before and after a batch which is similar to apart of our work. Their queue is not wait-free, in fact if the batch sizes are 1 then the queue is like MS-queue and it supports a new notion introduced by the authors called Extended Medium Futures Linearizability.

Nikolaev and Ravindran~\cite{DBLP:journals/corr/abs-2201-02179} present a wait-free queue that uses fast-path slow-path methodology. Their work is based on a circular queue using bounded memory. They design a slow phase which is wait-free bounded. If a process has no progress other processes try to help it in its \nf{CAS} loop in its slow phase. Since there can be $p$ processes helping the worst-case complexity cannot be better than linear.
...
Jayanti and Petrovic introduced a wait-free poly-logarithmic multi-enqueuer single-dequeuer queue~\cite{DBLP:conf/fsttcs/JayantiP05}. Their algorithm time complexity is $O(\log n)$ where $n$ is the number of enqueuers. We will improve their algorithm to construct a multi-enqueuer multi-dequeuer queue.

\bf{TODO: Where should be?}
Ellen and Woelfel introduced an implementation of a Fetch\&Inc object with step complexity of $O(\log p)$ using $O(\log n$)-bit \texttt{LL/SC} objects, where $n$ is the number of operations~\cite{10.1007/978-3-642-41527-2_20}. Their idea to achieve logarithmic complexity is to use a tree storing the list of elements added by processes and when a process wants to do Fetch\&Inc it appends its element to the tree and return the number of elements in the tree. Their work can be descibed a queue without dequeues.

\subsection{Universal Constructions}
Herlihy discussed the possibility of implementing shared objects from other objects~\cite{10.1145/114005.102808}. A \textit{universal construction} is an algorithm that can implement a shared version of any given sequential object. We can implement a concurrent queue using a universal construction. Jayanti proved an $\Omega(\log p)$ lower bound on the worst-case shared-access time complexity of $p$-process universal constructions~\cite{DBLP:conf/podc/Jayanti98a}. Chandra, Jayanti and Tan  introduced a semi-universal construction that achieves $\textsc{O}(\log^2 p)$ shared accesses~\cite{DBLP:conf/podc/ChandraJT98}. Their universal construction cannot be used to create any data structure, but a set of objects called closed objects. We mention a non-practical poly-logarithmic universal construction in page 12.
...
Attiya and Fouren proved a lower bound on amortized complexity of lock-free queues from the factor of contention (number of concurrent processes). Their result says any implementation of queues, using reads,
writes and conditional operations like \nf{CAS} has $O(\sqrt{\log \log p})$
amortized complexity \cite{DBLP:conf/opodis/AttiyaF17}. It is an open problem to create the reduce the gap between our algorithm and Attiya--Fouren lower bound.
