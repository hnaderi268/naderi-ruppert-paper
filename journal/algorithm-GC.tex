...
can be done in \rev{$O(1)$ steps}, without affecting the step complexity of other RBT operations.
...
\Function{void}{Propagate}{\typ{Node} \var{v}} \Comment{propagate blocks from \var{v}'s children to root}
    \If{{\keywordfont not} \Call{Refresh}{\var{v}}} \label{firstRefreshGC}  \hfill \Comment{double refresh: \rev{if first \op{Refresh} fails, try again}}
