...
\framebox[1.1\width]{TEST} How to merge notions of blocks and operations? block b $\in$ block c means b is subblock of c. block b $\in$ set B means b is in B. Merge these two to have shorter formulaes.
...
\begin{lemma}
  Each block is not duplicated.
\end{lemma}
\begin{proof}
  \nf{n.CreateBlock(i)} is successful for \nf{i=1,2,3,...} exactly one time and by definition of \nf{CreateBlock} the range of their blocks are disjoint so in propagation procedure on block can not be propagated twice (reference to propagation).
\end{proof}
...
If Line 313 of $r_1$ or $r_2$ returns \nf{true}, then the claim is held by Lemma~\ref{lem::trueRefresh}. If not, then there is another successful instance of \nf{n.Refresh()} $r_1^\prime$ by Lemma~\ref{lem:falseRefresh} for $r_2$. From $t_{i_{r_2}^\prime} > t_1$ and Lemma~\ref{lem::trueRefresh}, \ref{lem::establishedOrder} we know that \nf{ops(EST\sub{n.left, t\sub{1}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{1}})} $\subseteq$ \nf{ops(EST\sub{n, t\sub{2}})}.
