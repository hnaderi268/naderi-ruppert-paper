...
We now discuss the queue implementation in more detail.  Pseudocode is provided in Figure \ref{?}.
...
We use a similar abbreviation \var{num\sub{deq}}.
...
\Function{Object}{Dequeue()}{} 
    ...
    \State \Call{Append}{n}
    \State \var{<b, i>} \assign\ \Call{IndexDequeue}{leaf.\head, 1}
...
\Function{void}{Propagate}{\var{Node v}}
    \State \linecomment{propagates blocks from \var{v}'s children to root}
...
\Function{boolean}{Refresh}{\var{Node v}}
    \State \linecomment{tries to append a new block to \var{v.blocks}}
    ...
    \If{\var{new.num = 0}} \Return{\var{true}} \label{addOP} 
    \EndIf
...
\Function{void}{Advance}{\var{Node v}, \var{int h}} 
...
\Function{Block}{CreateBlock}{\var{Node v, int i}} 
    \State\linecomment{create new block to be installed in \var{v.blocks[i]}}
    \State let \var{new} be a new Block \label{initNewBlock}
    \ForEach{\var{dir} {\keywordfont{in}} \var{\{left, right\}}}
        \State \var{index\sub{prev} \assign\ v.blocks[i-1].\edir} \label{prevLine}
        \State \var{new.\edir\ \assign\ v.dir.\head-1} \label{lastLine}
        \State \linecomment{\var{new} contains \var{v.dir.blocks[index\sub{prev}+1..new.\edir]}}
        \State \var{block\sub{prev} \assign\ v.dir.blocks[index\sub{prev}]}
        \State \var{block\sub{last} \assign\ v.dir.blocks[new.\edir]}
    \EndFor
	\State \hangbox{\var{new.sum\sub{enq}} \assign\ \var{v.left.blocks[\eleft].sum\sub{enq} + \\
			v.right.blocks[\eright].sum\sub{enq}}}
	\State \hangbox{\var{new.sum\sub{deq}} \assign\ \var{v.left.blocks[\eleft].sum\sub{deq} + \\
			v.right.blocks[\eright].sum\sub{deq}}}
    \If{$\var{v} = \var{root}$}
        \State \hangbox{\var{new.size \assign\ max(0, \\ v.blocks[i-1].\size\ + new.num\sub{enq} - new.num\sub{deq})}\label{computeLength}}
    \EndIf
    \State \Return \tt{new}
...
\State \var{e} \assign\ \var{i + root.blocks[b-1].sum\sub{enq} - root.blocks[b-1].size}\label{computeE}
...
\Function{<int, int>}{IndexDequeue}{\sl{int} b, \sl{int} i} \cmt{Returns \tt{<x, y>} if $D_\tt{i}(\tt{this}, \tt{b}) \assign\ D_\tt{y}(\tt{root},\tt{x})$.}
\If{\tt{this} \bf{is} \tt{root}}
\State\Return \tt{<b, i>} \label{indexBaseCase}
\Else
\State \tt{dir\assign\ (parent.left=n ? left: right)} 
\State \tt{superblockIndex \assign\ parent.blocks[blocks[b].super].sum\sub{deq-dir} > blocks[b].sum\sub{deq} ? \label{computeSuper}
