...
\begin{lemma}[headPosition] \label{lem::headPosition} If the value of \nf{n.head} is \nf{h} then, \nf{n.blocks[i]=null} for \nf{i>h} and \nf{n.blocks[i]$\neq$null} for \nf{i<h}.
\end{lemma}
\begin{proof}
The value of \nf{n.head} is modified only in lines \ref{incrementHead1, incrementHead2} and a value is written into \nf{n.blocks[head]} is only in Line 313. It's obvious that writing into \nf{n.blocks[head]} does not change the state of the claim of the lemma and if it was right before 312 it is going to be right after 312. We show in both cases of the successful 313 or not the claim holds after the increment lines of \nf{n.head}. In the first case the process applied a successful \nf{TryAppend(new,h)} in line \ref{okcas}, which means \nf{n.blocks[h]} is not null anymore. If the claim was held before it is going to be held after the \ref{incrementHead1}. Since wether\ref{incrementHead1} returns true or false it means \nf{n.head} has been incremented. The failiure case is also the same since it means \nf{n.blocks[head]} is written by some process.
...
If Line 313 of $r_1$ or $r_2$ returns \nf{true}, then the claim is held by Lemma~\ref{lem::trueRefresh}. If not, then there is another successful instance of \nf{n.Refresh()} $r_2^\prime$ which has did TryAppend successfully into \nf{n.blocks[i+1]}. In figure 1 we see why the block it is appending contains established block in the n's children at $t_i$, since it create a block reading the head after $t_1$.
...
\item We call operations strictly before $op$ in the sequence of operations $S$, prefix of the $op$.
  \item $E(n,i)$ is the sequence of enqueue operations $\in$ \nf{ops(n.blocks[i])} ordered by their process id.
  \item $D(n,i)$ is the sequence of dequeue operations $\in$ \nf{ops(n.blocks[i])} ordered by their process id.
...
\begin{lemma}[Index correctness]
 \nf{n.Index(b,i)} returns the rank in $D(root)$ of the $i$th Dequeue in $D(n,b)$.
\end{lemma}
%\begin{proof}
...
After computing line \ref{computeSuper} of \nf{n.IndexDEQ(b,i)}, \nf{superblock} contains the \nf{i}th dequeue in the \nf{b}th block of the node \nf{n}.
\end{lemma}
%\begin{proof}
%\begin{enumerate}
