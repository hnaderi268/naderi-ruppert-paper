...
\hspace{\parindent}First, I want to thank my supervisor Eric Ruppert. Without his guidance and support, this thesis probably would not have been completed. He helped me in every step of this thesis, taught me the shared data structures, helped me find the problem, shape up the algorithm, and write it with its complications because of lots of new notions in it. Thank you again for always pushing to make it better.
...
Last but not the least, I would like to thank my parents for providing me with unfailing support and continuous encouragement throughout my years of study. I want to thank my brother for being there whenever I needed help.

I would like to express my gratitude to York University for the scholarship I was awarded in 2021-2022 academic year.

I dedicate this paper to all the people who are fighting for their freedom.
...
We present our algorithm in pseudocode. Page~22 contains the description of the fields in the tree nodes and the blocks. Value of any uninitialized field is \nf{null}. Page~23 contains major routines and the rest of this section consists of the auxiliary routines
...
\begin{corollary}\label{blockSize}
    Each block contains at most $c$ operations in total, where $c$ is the number of concurrent processes ($c\leq p$).
\end{corollary}

\begin{lemma}\label{subBlocksBound}
Each block has at most $c$ direct subblocks ($c\leq p$).
\end{lemma}
\begin{proof}
From Definition \ref{def::ops} we know the operations in a block are the union of the operations in the direct subblocks of the block. We can see that each block appended to an internal node contains at least one operation due to the test on Line~\ref{addOP}. Also, blocks in the leaves contain only one \nf{Enqueue} or \nf{Dequeue} operation. By Corollary \ref{blockSize} each block in an internal node contains at most $c$ operations and each one of its direct subblocks has at least one operation, so by pigeonhole principle the number of direct subblocks in a block is at most $c$.
\end{proof}
...
\begin{observation}
    If the number of concurrent processes in an execution is $c$, then the total amortized worst-case step complexity would be $O(\log p\log c + \log q)$. Furthermore, in a sequential execution where $c=1$, the step complexity of our algorithm would be $\Theta(\log p + \log q)$.
\end{observation}
\begin{proof}
    The analysis is similar ro two previous Lemmas, but by Lemma \ref{subBlocksBound} each \nf{GetEnqueue} takes $O(\log c)$ steps.
\end{proof}
