...
\State \tt{CAS(dir.blocks[h\sub{dir}].super, null, h)}
\State \tt{CAS(dir.head, h\sub{dir}, h\sub{dir}+1)}
\EndIf
...
\EndIf
\State{\tt{result= blocks.tryAppend(new, h)}} \label{cas}
\State \tt{hp= parent.\head} \label{readParentHead}
\State \tt{\Call{CAS}{n.blocks[h].super, null, hp}} \label{setSuper1}
...
\pagebreak
\begin{definition} We say \nf{R1} an instance of \nf{Refresh} is before \nf{R2} another instance of \nf{Refresh}. If \nf{R1}'s \nf{TryAppend} happens before \nf{R2}'s \nf{TryAppend}. We knew that \nf{CAS} objects are linearizable.
\end{definition}
