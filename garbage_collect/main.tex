...
One way of handling garbage collection is to reallocate the space taken by each each operation right after it is not needed anymore, but it might cost too much to do that. So we garbage collect by batching. If we do the garbage collection every $p^2$ block appended to the root, its cost is amortized over $p^2$ blocks which is $O(p^3)$ and $\Omega(p^2)$.

In our design a process attempts to do \nf{Garbage Collect} every $p^2$ block appended to the root. \nf{Garbage Collect} corresponded to the $kp^2$ root block is called the $k$th round. To know if it is $k$th round or nor we can use an if condition on the value of \nf{root.head}. But the problem is that a process might miss a round not because it is on idle but because it reads \nf{root.head} not to be $kp^2$. We handle that by checking with a window.
...
\begin{lemma}
When $\cfrac{\nf{root.head}}{p^2}$ incremnts from $r$ to $r+1$, at least one operation has executed Lines 48,49.
\end{lemma}

An \nf{Enqueue} operation cannot be remove after it is terminated since later it might be dequeues, however it can be remove from the tree after it has been computed to be the response of a \nf{Dequeue}. A \nf{Dequeue} operation can be removed from the tree safely after it has computed its response \nf{Enqueue}. Note that it is not needed for the \nf{Dequeue} to be terminated but only it is sufficient if it has computed its response. We say a block in the \nf{root} is \it{finished} if all of its operations can be removed.

\begin{lemma}
There are at most $p^2+q$ not finished blocks in the \nf{root.blocks} at a time.
\end{lemma}

The good property in a fifo queue is that if $i$th \nf{Enqueue} gets dequeued then it means $1$ to $i-1$th enqueue operation have also dequeued. This gives us the idea that if a block is finished all the blocks before it are also finished. However if an operation in a block goes to sleep for a long time then the block remains not finished. Since there are at most $p$ idle operations we can help them and then remove all the finished blocks safely.

\begin{lemma}
    If all current operations are helped, then there is a block in the root that all of its previous blocks are finished.
\end{lemma}

The idea above leads us to a poly-log data structure that supports throwing away all the values smaller than an index. Red-black trees do this for us.
We can create a shared red-black tree just creaing a new path for the operation and then using \nf{CAS} to change the root of the tree. See [this] for more.
\begin{observation}
PBRT supports....
\end{observation}
...
But how can we know which blocks in each node are finished or not? We can keep track of the last finished block.
...
There is a shared array among processes which they write the last block dequeud from in it. 


\begin{lemma}
    \nf{lastDequeuedFrom}- index of the last finished block in the root is $O(p)$.
\end{lemma}

\begin{lemma}
    After \nf{FreeMemory}, the space taken by each node is $O(p^2+q)$
\end{lemma}
