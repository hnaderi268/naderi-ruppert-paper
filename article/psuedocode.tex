...
\ElsIf{\tt{blocks.tryAppend(new, s)}} \label{cas}
...
\EndFor
\State \tt{\Call{CAS}{size, s, s+1}} \label{incrementHead1}
\State \Return{\tt{true}}
\Else
\State \tt{\Call{CAS}{size, s, s+1}} \cmt{Even if another process wins, help to increase the size. The winner might have fallen sleep before increasing \tt{size}.} \label{incrementHead2}
...
\framebox[1.1\width]{TEST} As a temporary test I have changed the name of n.size to n.head here, other options are n.head and n.lastBLock but they might be confusing since we have used them before. Fix the logical order of definitions (cyclic refrences).

\framebox[1.1\width]{TODO} Fallback safety lemmas. Some parts are obsolete. 
...
A block is an object storing some statistics described in Algorithm Queue. It implicitly shows a set of operations. The set of operations of block $b$ are the operations in the leaf subblocks of $b$. We show the set of operations of block $b$, set of blocks $B$ by $ops(b)$, $ops(B)$. We also say $b$ contains $op$ if $op\in ops(b)$.
\end{definition}
...
\begin{definition}
  Block \nf{b} in \nf{n.blocks} is \emph{Established} at time $t$ if \nf{n.head} is greater than index of \nf{b} at time $t$. Block \nf{b} is in \emph{EST\sub{n, t}} if \nf{b} is a subblock of \nf{b$^\prime$} in \nf{n.blocks} such that \nf{b$^\prime$} is established  at time $t$.
\end{definition}


\begin{observation}\label{head}
Once block b is written in \nf{n.blocks[i]} then \nf{n.blocks[i]} never changes.
\end{observation}



\begin{lemma}[headProgress] \label{lem::sizeProgress}
\nf{n.head} is non-decreasing over time and \nf{n.blocks[i].end\sub{left, right}} $\geq$ \nf{n.blocks[i-1].end\sub{left, right}}.
\end{lemma}
\begin{proof}
  Induced trivially from the pseudocode since \nf{n.head} is only incremented. \nf{n.blocks[i]. Also end\sub{left, right}} are greater than or equal the previous values in the \nf{CreateBlock(i)} code (Lines~\ref{lastLine, prevLine}).
\end{proof}
...
\end{lemma}
\begin{proof}
To show this we are going to refer to the way n.blocks[] is partitioned while propagating up to n.parent. \nf{n.CreateBlock(i)} merges the blocks in \nf{n.left.blocks[n.blocks[i-1].end\sub{left}..n.blocks[i].end\sub{left}]} and \nf{n.right.blocks[n.blocks[i-1].end\sub{right}..n.blocks[i].end\sub{right}]} (Lines \ref{lastLine, pr}). From we know that \nf{end\sub{left},end\sub{right}} are non decreasing, so the range of the subblocks of n.blocks are disjoint.
\end{proof}

\begin{corollary}[No Duplicates]\label{append}
If \nf{op} is appended to \nf{n.blocks[i]} then after that there is no \nf{j>i} such that \nf{op}$\in$\nf{ops(n.blocks[j])}.
\end{corollary}
%\begin{proof}
...
\end{invariant}
\begin{proof}
Some value is written into \nf{n.blocks[head]} only in Line 313. It is obvious that writing into \nf{n.blocks[head]} does not change the state of the claim of the lemma. The value of \nf{n.head} is modified only in lines \ref{incrementHead1}, ][p\ref{incrementHead2}, we show in both cases of the successful \ref{cas} or not the claim holds after the increment lines of \nf{n.head}. If $h$ increments to $h$ it is sufficient to show \nf{n.blocks[h]}$\neq$\nf{null} to claim the invariant still holds. In the first case the process applied a successful \nf{TryAppend(new,h)} in line \ref{okcas}, which means \nf{n.blocks[h]} is not null anymore. Note that wether\ref{incrementHead1} returns true or false after Line \nf{n.head} we know has been incremented from Line \ref{readSize}. The failure case is also the same since it means some value is written into \nf{n.blocks[head]} by some process.
%At the end of every \nf{n.Refresh()} with a block  with size greater than 0 returned by \nf{CreateBlock()} \nf{n.head} is incremented (Lines \ref{incrementHead1}, \ref{incrementHead2}). If a process went to sleep before incrementing the \nf{established} (line \ref{incrementHead1}), nothing can be appended to \nf{n.blocks} until another process increments \nf{n.establsihed} (Line \ref{incrementHead2}).
\end{proof}
...
\end{lemma}
\begin{proof}
Blocks are only appended(not modified) with CAS to \nf{n.blocks[n.head]} and \nf{n.head} is non-decreasing, so the set of operations in established blocks of a node grows.
\end{proof}
...
%% WLOG we prove the claim for the left child. Blocks in \nf{n.left.blocks[n.blocks[i-1].end\textsubscript{left}+1..n.blocks[i].end\textsubscript{left}]} are established at time $t$.  Since the head is only increasing (Lemma~\ref{lem::sizeProgress}) the  claim holds after $t$. See Figure~\ref{fig::createBlock}. The right child is the same.
%\end{proof}

\begin{lemma}[trueRefresh] \label{lem::trueRefresh}
  Let $t_i$ be the time \nf{n.Refresh()} is invoked and $t_t$ be the time it is terminated. Suppose \nf{n.Refresh()}'s \nf{TryAppend(new, s)} returns \nf{true}, then \nf{ops(EST\sub{n.left, t\sub{i}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{i}})} $\subseteq$ \nf{ops(EST\sub{n, t\sub{t}})}.
\end{lemma}
\begin{proof}
From Lemma \ref{lem::establishedOrder} we know that \nf{ops(EST\sub{n, t\sub{i}})}$\subseteq$\nf{ops(EST\sub{n, t\sub{t}})}. So it remains to show \nf{ops(EST\sub{n.left, t\sub{i}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{i}})} - \nf{ops(EST\sub{n, t\sub{i}})} which we call new operations $\subseteq$\nf{ops(EST\sub{n, t\sub{t}})}. If \nf{TryAppend}returns true a block is appended to n. From the code of the \nf{CreatBlock} this block includes the establsihed blocks in n's children at $t_i$. Since the head in createblock is read after $t_i$. So the new operations are inthe block appended to $n$.
...
  Consider two consecutive failed instances $r_1$,$r_2$ of \nf{n.Refresh()} by some process. Let $t_1$ be the time $R_1$ is invoked and $t_2$ be the time $R_2$ terminated. After $R_2$'s \nf{TryAppend} we have \nf{ops(EST\sub{n.left, t\sub{1}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{1}})} $\subseteq$ \nf{ops(EST\sub{n, t\sub{2}})}.
\end{lemma}
\begin{proof}
  

If Line \ref{cas} of $R_1$ or $R_2$ returns \nf{true}, then the claim is held by Lemma~\ref{lem::trueRefresh}. If not, then there is another successful instance of \nf{n.Refresh()} $R_2^\prime$ which has did TryAppend successfully into \nf{n.blocks[i+1]}. Note that if $R_2$ reads some value greater than $i+1$ in Line \ref{readSize} it means there is a succesful \nf{Refresh()} instance started after Line \ref{readSize} of $R_1$ and finished its Line \ref{incrementHead1} or \ref{incrementHead2} before \ref{readSize} of $R_2$, form Lemma~\ref{lem::trueRefresh} by finish of this instance \nf{ops(EST\sub{n.left, t\sub{1}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{1}})} has been propagated. In the figure 1 we see why the block $R_2^\prime$ is appending contains established block in the n's children at $t_i$, since it create a block reading the head after $t_1$.
...
Each block contains at most one operation from each processs \nf{($\forall$ \nf{process p, block b:}\#\nf{operations of p}$\in$\nf{ops(b)$\leq$1}}.
\end{lemma}
\begin{proof}
Assume there are two operations $op_1, op_2$ from process $p$ in block $b$. A process cannot invoke two operations concurrently. WLOG assume \nf{leaf of p.Append(block containing op\sub{1})} has to be finished before \nf{leaf of p.Append(block containing op\sub{2})} starts. So before appending $op_2$ to the tree $op_1$ exists in every node. So there is a node that has two blocks containing $op_1$ since if not then $b$ cannot exist. This contradicts with \ref{append}.
\end{proof}
\textit{Is something wrong with this proof? It is more complicated than I thought.}
...
\nf{n.GetENQ(b,i)} returns the $i$th Enqueue in $E(n,b)$.
\end{lemma}
\begin{proof}
It is easy to see that \nf{leaf.GetENQ(b,1)} returns the operation stored in the $b$th block of leaf $l$. In Line 404 it is decided that the \nf{i}th enqueue in block \nf{b} of internal node \nf{n}resides in the left child or the right child of \nf{n}. Then \nf{n.child.GetENQ( block containing, order in the block)} is invoked. This recursive procedure ends in a leaf node. After that Lines 88, 92 search the proper subblocks of \nf{b}. The correctness of the invocation parameters is induced from the definition of ops(block), subblock.
\end{proof}
...
\begin{lemma}[Computing Queue's Head] \label{computeHead}
  Let $S$ be the state of an empty queue if the operations in prefix in $L$ of $i$th dequeue in $D(root,b)$ are applied on it. \nf{FindResponse()} returns the index in $E(root,b)$ of the enqueue that is the head in S. If the  queue is empty in S it returns \nf{<-1,-->}.
  \end{lemma}
...
\begin{lemma}[Root search range]\label{rootRange}
  \nf{root.size-root.FindMostRecentDone()} is $O(p^2+q)$, which $p$ is \# processes and $q$ is the length of the queue.
\end{lemma}
