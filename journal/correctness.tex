...
We first prove the \fld{super} field is accurate, since \op{IndexDequeue} uses it to trace superblocks up the tree.  We do this by showing that the \fld{super} field of a block $B$ in node $\var{v}$ is read from  
\var{v}.\fld{parent}'s \var{head} field close to the time that $B$'s superblock $B_s$ is installed in $v$'s parent.
...
By the definition of subblocks (\ref{defsubblock}), $R_s$'s read $r$ of $\var{v}.head$ at line \ref{createEndLeft} %or \ref{createEndRight} 
