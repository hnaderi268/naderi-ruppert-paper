...
if some block containing the operation has previously appeared in the RBT \var{v}.\fld{block}
...
then the claim follows from the assumption that Invariant \ref{inv::finished} holds at all times before $st$.
...
then it has been propagated to the root and its result has been written in the \fld{response} field of the leaf block it created.
...
Let \rev{$t$ be the time} when $A$ invokes \op{SplitBlock} on line \ref{callSplitBlock}.
...
$\ell$ is the last block added to the root's RBT before $t$.
...
at time $t$; otherwise the process that performed that \dequeue\ would have set its \var{last} entry
to the index of the root block that contains the \enqueue, which is greater than $m$, before $t$, contradicting
the fact that all values in the $last$ array at $t$ are less than or equal to $m$.
...
at $t$; otherwise the process that performed the \dequeue\ would have set its \var{last} entry
to a value greater than the index of the root block that contains the \dequeue\ prior to $t$, 
...
%\here{At first, I thought the following analysis could be tightened
%by using the fact that operations only appear along one path in tree
%so doing a bound on total size of all nodes on one level might work yield better bound.
%But I don't think so, because we could have each process go in order to dequeue
%everything and then enqueue q new items (where q>>p).  Then sets of enqueue operations
%stored in each node at one level of tree would be disjoint (because of the way
%GC is timed differently at each node).
...
Either way, by Lemma \ref{RBTupdates}, \var{v}'s RBT's maximum \fld{index} will be at least $r\cdot G$ at all times
after this CAS.
