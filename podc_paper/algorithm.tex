...
To simplify the code, each \node\ has an empty \block\ $B_0$ in \fld{blocks}[0].
...
the block in $v.\fld{blocks}[b]$ has two fields \fld{sum\sub{enq}} and \fld{sum\sub{deq}}
...
In this case, \op{Refresh} returns true at line \ref{addOP} and terminates.
...
\nf{Dequeue}. 
A binary search is used on the \fld{sum\sub{enq}} fields of \var{root}.\fld{blocks} to find the index $b_e$ of the block that contains 
the \var{e}th enqueue.
The index $b_e$ is at most $b$ (since the enqueue is linearized before the dequeue).  To find the left end of the range for the binary search for $b_e$, we can first do a doubling search \cite{BY76}, comparing \var{e} to the \fld{sum\sub{enq}} fields at indices $b-1, b-2, b-4, b-8, \ldots$.
