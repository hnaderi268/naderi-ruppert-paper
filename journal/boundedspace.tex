...
This \rev{increases the running time of any routine designed for a (sequential) RBT only by a constant factor, assuming that nodes are accessed only by following pointers from the root (as is the case for all RBT operations we require)}.
...
also keep the running time of our operations polylogarithmic \rev{in $p$ and $q$}.
