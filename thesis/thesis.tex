...
\huge{\bf{A Wait-free Queue with Poly-logarithmic\\Worst-case Step Complexity}}

\vspace{2em}
\Large{\bf{Hossein Naderibeni}}

\vspace{3em}
\large A Thesis submitted to the Faculty of Graduate Studies \\ in Partial Fulfillment of the Requirements\\for the Degree of Master of Science
...
\vspace{3em}
\textcopyright Hossein Naderibeni, 2022
...
First, I want to thank my supervisor Eric Ruppert. ...

I would also like to thank professor Franck van Breugel. ...

I am grateful to all members of my examining committee for their time and their helpful comments on my thesis.
...
\begin{figure}[hbt]
  ...
  \caption[An example of a linearizable execution.]{\label{fig::goodexample}An example of a linearizable execution. Either \texttt{Enqueue(A)} or \texttt{Enqueue(B)} could take effect first since they are concurrent.}
\end{figure}

\begin{figure}[hbt]
  \center\includegraphics[scale=0.5]{pics/bad}
  \caption[An example of a non-linearizable execution.]{\label{fig::badexample}An example of an execution on an empty queue that is not linearizable. \texttt{Enqueue(A)} has terminated before \texttt{Enqueue(B)} is invoked and the \texttt{Dequeue} can occur before \texttt{Enqueue(A)} and return \nf{null} or can occur after \texttt{Enqueue(A)} and return \texttt{A}.}
\end{figure}
...
In this section, we look at previous lock-free queues. In the worst-case, step complexity of all the lock-free queues includes an $\Omega(p)$ term that comes from the case where all $p$ processes try to do an enqueue or a dequeue simultaneously. Morrison and Afek call this the \textit{CAS retry problem}~\cite{DBLP:conf/ppopp/MorrisonA13}. 
...
\begin{figure}[hbt]
  ...
  \caption[MS-queue structure]{\label{fig::msq}MS-queue structure, enqueue and dequeue operations. In the first diagram, the first element has been dequeued. Red arrows show new pointers and gray dashed arrows show the old pointers.}
\end{figure}
...
\begin{figure}[hbt]
  \center\includegraphics[scale=0.5]{pics/sundell}
    \caption[Global and local pointers in~\cite{DBLP:conf/opodis/GidenstamST10}]{\label{fig::sundell}Global pointers point to arrays. Head and Tail elements are blue, dequeued elements are red and current elements of the queue are green.}
\end{figure}

Kogan and Petrank~\cite{DBLP:conf/ppopp/KoganP11} introduced wait-free queues based on the MS-queue and use Herlihy's helping technique~\cite{10.1145/114005.102808} to achieve wait-freedom. Their step complexity is $\Omega(p)$ because of the helping mechanism.
...
The CAS Retry Problem is not limited to list-based queues; array-based queues also share it as well~\cite{DBLP:conf/iceccs/ColvinG05,DBLP:conf/icdcn/Shafiei09,DBLP:conf/spaa/TsigasZ01}.  Our motivation is to overcome this problem and present a wait-free sublinear queue.
...
We present our algorithm in pseudocode. Page 23 contains the description of the fields in the tree nodes and the blocks. Value of any uninitialized field is \nf{null}. Page 24 contains major routines and the rest is the auxiliary routines
The abbreviations bellow are used in the pseudocode and the proof of correctness.
\begin{itemize}
 \item \tt{blocks[$b$].sum\sub{x}=blocks[$b$].sum\sub{x-left}+blocks[$b$].sum\sub{x-right}} (for internal blocks where $b\geq 0$ and \tt{x} $\in$ \{enq, deq\})
...
\Statex $\triangleright$ \textmd{Does a binary search for~the value \tt{x} of \tt{sum\sub{enq}} field and returns the index of the leftmost block in \tt{blocks[start..end]} whose \tt{sum\sub{enq}} is $\geq$ \tt{x}}.
\Function{int}{BinarySearch}{\sl{int} x, \sl{int} start, \sl{int} end}
% \State \Return \tt{min\{j: blocks[j].sum\sub{enq}$\geq$x\}}
\While{\nf{start<=end}}
\State \sl{int} \tt{mid= (start+end)/2}
...
Here we want to demonstrate how four processes help each other to  propagate their operations up to the root and then compute their responses.

Initially, each node in the tree contains one empty block. In Figure \ref{fig::exec1}, processes $P1,P2,P4$ have appended the blocks containing $ENQ_1,DEQ_2,DEQ_4$. Line \ref{appendLeaf} appends a block to the process's leaf.

Then as in Figure \ref{fig::exec2}, $P2$ does a successful \nf{Refresh} on the node $n5$ that propagates both the operations pf $P1$ and $P2$. $P4$ does also a successful \nf{Refresh} on the node $n6$ and propagates $DEQ_4$. $P1$'s \nf{Refresh} on $n5$ is slower than $P2$'s and creates its new block after $P2$'s successful \nf{CAS} that propagated $ENQ_1$ and $ENQ_2$. So the block created by $P1$ is empty as there is nothing to propagate ($P1$'s Line \ref{readHead} is after $P2$'s Line \ref{cas}).

Since the block that $P1$ creates is empty, it returns \nf{true} in Line \ref{addOP} as shown in Figure \ref{fig::exec3}. $P3$ propagates $ENQ_3$ to $n6$ and $P4$ is very fast and it propagates $DEQ_4$ to the \nf{root} even before $P2$ puts its block into $n5$.

Then as shown in Figure~\ref{fig::exec4}, $P4$ goes to sleep, and $P2$ helps to increment \nf{root.head} after its failed \nf{CAS} on the root. After incrementing \nf{root.head}, $P3$ succeeds to do \nf{root.Refresh} with a single \nf{CAS} on the \nf{root} and puts operations from $P1,P2$ and $P3$ into the \nf{root}. $P1$ fails to do \nf{Refresh} on the \nf{root} two times, but it is guaranteed that its operation is propagated before its second failed \nf{CAS} on \nf{root.blocks}.

All the fields of selected blocks from Figure \ref{fig::exec4} are shown in Figure \ref{fig::exec5}. $P1$ and $P3$'s operations terminated after that they appended the \nf{Enqueue}s to the root. $P2$ needs to compute the index of the block that its \nf{Dequeue} is in. It computes the superblock of its leaf block, which is the second block in $n5$. Then, $P2$ computes the superblock of the block containing $ENQ_1,DEQ_2$. As we can see the \nf{super} field of  $ENQ_1,DEQ_2$ is 1 but the index of its superblock is 2. The queue size after the previous block is 0 so $DEQ_2$ 's response is $ENQ_1$'s element. Then $P2$ tries to get $ENQ_1$'s element. $ENQ_1$ is in the left subblock of  block $ENQ_1,ENQ_3,DEQ_2$ because 1 is less than the \nf{.sum\sub{enq-left}} field of the block containing $ENQ_1,ENQ_3,DEQ_2$. Similarly , $P2$ determines that $ENQ_1$ came from the left child of $n5$. $P_2$ therefore reaches the leaf $n1$ and returns $ENQ_1$'s element. $P4$ similarly finds the block in the \nf{root} that contains $DEQ_4$. Since it is the first operation in the block and the size of the queue after the previous (empty) block is 0, $DEQ_4$ returns \nf{null}. 

\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec1.png}
  \caption{$P1,P2,P4$ append operations to their leaves. \label{fig::exec1}}
\end{figure}

\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec2.png}
  \caption[$P2$ helps $P1$. $P4$ is faster than $P3$.]{In the diagram, we use $n[i]$ as an abbreviation for $n$\nf{.blocks[$i$]}. $P2$ helps $P1$. $P4$ is faster than $P3$. \label{fig::exec2}}
\end{figure}


\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec3.png}
  \caption{$P4$ puts its block into the \nf{root} before $P2$ appends its block to $n5$.\label{fig::exec3}}
\end{figure}

\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec4.png}
  \caption[$P3$'s \nf{CAS} on \nf{root.blocks} succeeds.]{$P3$'s \nf{CAS} on \nf{root.blocks} succeeds. $P1$ and $P2$'s operations are done, each after two failed \nf{CAS} attempts on \nf{root.blocks}. \label{fig::exec4}}
\end{figure}
...
A block $B$ in a leaf represents an \nf{Enqueue} if $B\nf{.element}\neq\nf{null}$. Otherwise, if $B\nf{.element}=\nf{null}$, $B$ represents a \nf{Dequeue}. The set of operations of block $B$ is the union of the operations in leaf subblocks of $b$. We denote the set of operations of block $B$ by ${ops(B)}$ and the union  of operations of a set of blocks \bf{B} by $ops(\bf{B})$.  We also say $B$ contains ${op}$ if $op\in{ops(B)}$.
\end{definition}
...
\begin{align*}
    ...
    =& \Big|E(n.\nf{left},B^\prime.\nf{end\sub{left}}+1)\cdots E(n.\nf{left},B.\nf{end\sub{left}})\Big|
\end{align*}
First line follows from the Definition of \nf{num\sub{enq}}. Second line is similar to the way \nf{sum\sub{enq-left}} is computed in the \nf{CreateBlock} routine. \text{Observation } \ref{sumToNum} results the third line and the last line holds because of the induction hypothesis (3). (2) is similar to (1). Now we prove (3) starting from the Definition of $E(n,b)$.
\begin{align*} 
E(n,b) =&  E(n\nf{.left},n\nf{.blocks[}b-1\nf{].end\sub{left}}+1)\cdots E(n\nf{.left},n\nf{.blocks[}b\nf{].end\sub{left}})\cdot \\ 
&E(n\nf{.right},n\nf{.blocks[}b-1\nf{].end\sub{right}}+1)\cdots E(n\nf{.right},n\nf{.blocks[}b\nf{].end\sub{right}}).
\end{align*}
...
\end{corollary}
\begin{proof}
  If the first \nf{Refresh} in line \ref{firstRefresh} returns \nf{true}, then by Corollary \ref{lem::prectrueRefresh} the claim holds. If the first \nf{Refresh} failed and the second \nf{Refresh} succeeded, the claim still holds by Corollary \ref{lem::prectrueRefresh}. Otherwise, both failed, and the claim is satisfied by Lemma \ref{doubleRefresh}.
\end{proof}
...
\end{lemma}
\begin{proof}
From the definition of operations in a block in Definition \ref{def::ops} we know the operations in a block are the union of the operations in the direct subblocks of the block. The claim follows from the equivalence of the number of operations in a block and the number of operations in the subblocks of the block and the observation that each block appended to an internal node contains at least one operation due to the test on Line~\ref{addOP}. Also, blocks in the leaves contain only one \nf{Enqueue} or \nf{Dequeue} operation.
\end{proof}
...
\end{lemma}
\begin{proof}
We will prove this lemma by induction on the node's height $n$. For the base case, suppose $n$ is a leaf. Leaf blocks each contain exactly one operation, $n\nf{.blocks[$b$].sum\sub{enq}}\leq 1$, which means only \nf{$n$.GetEnqueue(b,1)} can be called when $n$ is a leaf and contains and \nf{Enqueue} operation. Line \ref{getBaseCase} of \nf{$n$.GetEnqueue($b$, $1$)} returns the \nf{element} of the \nf{Enqueue} operation stored in the $b$th block of leaf ${n}$, as required.
...
Let $subblock$ be the block in $n.\nf{dir}$ containing $E_i(n,b)$. Finally, \nf{$n$.child.GetEnqueue($subblock$, $i$)} is invoked  and it returns $E_i(n,b)$\nf{.element} by the hypothesis of the induction.
\end{proof}
...
As we talked about earlier in Section 2.4 the Attiya--Fouren lower bound says that a concurrent implementation of queues using reads, writes and conditional operations like \nf{CAS} has $\Omega(c)$ amortized complexity \cite{DBLP:conf/opodis/AttiyaF17} when $c$, the number of concurrent processes, is $O(\log\log p)$. Our amortized complexity is $O(\log^2p+\log q)$. It is an open problem to reduce the gap between our algorithm and the Attiya--Fouren lower bound.
