...
\Eric{In the thesis, Lemma 52 said enqueues take $O(\log^2 p)$ steps.  Was this a typo?}
...
leaf and propagates it to the root.  The \op{Propagate} routine does $O(1)$ steps 
at each node along the path from the process's leaf to the root.
A null \op{Dequeue} additionally calls \op{IndexDequeue}, which also does $O(1)$ steps
at each node along this path. 
So, the total number of steps for either type of operation is $O(\log p)$.

A non-null \op{Dequeue} must, in addition, do a search at line \ref{FRb} and call \op{GetEnqueue}
at line \ref{findAnswer}.
\Eric{Modified:}
By \Cref{dSearchTime}, the search takes $O(\log(q_e+q_d+p))$ steps,
since the size of the queue can change by at most $p$ within one block of operations
(by \Cref{blockSize}).
\op{GetEnqueue} does a binary search within each node along a path from the root to a leaf.
The search in each node $v$ is within the subblocks of one block in $v$'s parent.
By \Cref{blockSize}, each search takes $O(\log c)$ time, for a total of $O(\log p\log c)$ steps.
\end{proof}

\Eric{I left out the amortized bound, since the worst-case bound seems stronger}
