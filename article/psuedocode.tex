...
\EndFor
\State \tt{\Call{CAS}{\head, s, s+1}} \label{incrementHead1}
\State \Return{\tt{true}}
\Else
\State \tt{\Call{CAS}{\head, s, s+1}} \cmt{Even if another process wins, help to increase the \head. The winner might have fallen sleep before increasing \tt{\head}.} \label{incrementHead2}
...
\State \tt{newBlock.\size= max(root.blocks[i-1].\size + b.num\sub{enq} - b.num\sub{deq}, 0)}\label{computeLength}
...
\framebox[1.1\width]{TEST} As a temporary test I have changed the name of n.\head to n.head here, other options are n.head and n.lastBLock but they might be confusing since we have used them before. Fix the logical order of definitions (cyclic refrences).
