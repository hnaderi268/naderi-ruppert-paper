...
If a process performing a \op{Refresh} on the root finds that $\var{root}.\head$ is a multiple
of $p^2$, it does GC.
...
\var{root} containing an enqueue whose element it dequeued.
...
Then, GC can work its way down the tournament tree, using \op{Split} operations to discard all subblocks
of the discarded root blocks.

\here{Have to talk about repeating CAS to do GC at each node up to p times}
