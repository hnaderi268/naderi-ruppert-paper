...
\begin{itemize}
  	\item \typ{int} \fld{sum\sub{enq}, sum\sub{deq}}
  		\Comment{number of enqueues, dequeues in \fld{blocks} array up to this block (inclusive)}
...
\Function{boolean}{Refresh}{\typ{Node} \var{v}} \Comment{try to append a new block to \var{v}.\fld{blocks}}
...
\Function{Block}{CreateBlock}{\typ{Node} \var{v}, \typ{int} \var{i}} \Comment{create new block for \op{Refresh} to install in \var{v}.\fld{blocks}[\var{i}]}
    \State \hangbox{let \var{new} be a new \typ{Block} with fields\label{initNewBlock}
    	\eleft\ \assign\ $\var{v}.\fld{left}.\head - 1$,\label{createEndLeft}
    	\eright\ \assign\ $\var{v}.\fld{right}.\head - 1$,\label{createEndRight}\\
		\fld{sum\sub{enq}} \assign\ \var{v}.\fld{left.blocks}[\var{new}.\eleft].\fld{sum\sub{enq}} + \var{v}.\fld{right.blocks}[\var{new}.\eright].\fld{sum\sub{enq}}\label{createSumEnq},\\
		\fld{sum\sub{deq}} \assign\ \var{v}.\fld{left.blocks}[\var{new}.\eleft].\fld{sum\sub{deq}} + \var{v}.\fld{right.blocks}[\var{new}.\eright].\fld{sum\sub{deq}}}\label{createSumDeq}
    ...
    \If{$\var{v} = \var{root}$} \var{new}.\fld{size} \assign\ max(0, $\var{v}.\fld{blocks}[\var{i}-1].\size\ + \var{num\sub{enq}} - \var{num\sub{deq}}$)\label{computeLength}
    \EndIf
...
\Function{$\langle\typ{int}, \typ{int}\rangle$}{IndexDequeue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}} \com\ return $\langle\var{b}', \var{i}'\rangle$ such that \var{i}th dequeue in $D(\var{v}.\fld{blocks}[\var{b}])$
...
\Function{element}{GetEnqueue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}} \Comment{returns argument of \var{i}th enqueue in $E(\var{v}.\fld{blocks}[\var{b}])$}
    ...
    \Else 
        ...
        \State \linecomment{find enqueue's block in \var{v}.\fld{dir} and its rank within block}
