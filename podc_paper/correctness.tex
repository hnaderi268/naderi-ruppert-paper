...
routines correctly navigate through the tournament tree.
Finally, we show that the implementation is linearizable in Section \ref{sec::linearizability}.

\subsection{Basic Properties}
\label{sec::basicProperties}

\Eric{I'm using immutable to mean that they are written only when the object is created.  The \fld{super} field does change from null to some value after the block is created, so it 
is written once, rather than 0 times.  In other words, we could have initialized
 all the other fields when the Block constructor is called.}
