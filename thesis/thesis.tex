...
A call to \nf{$n$.GetEnqueue(b,i)} returns the \nf{element} of the \nf{i}th enqueue in \nf{b}th the block of $n$. The range of subblocks of a block is determined using the \nf{end\sub{left}} and \nf{end\sub{right}} fields of the block and its previous block. Then, the subblock is found using binary search on the \nf{sum\sub{enq}} field (Lines \ref{leftChildGet} and \ref{rightChildGet}).

\paragraph{\tt{IndexDequeue($b,i$)}}

$n$\nf{.IndexDequeue($b,i$)} computes the block and the rank within the block of the $i$th \nf{Dequeue} of $b$th block of $n$ in the root. Let $R_n$ be the successful  \nf{Refresh} on node $n$ that did a successful \nf{CAS(null, $B$)} into \nf{n.blocks[$b$]}. Let $n$\nf{.parent} be $n$\nf{.parent}. Without loss of generality assume for the rest of this section a ${n}$ is the left child of $n$\nf{.parent}. Let $R_p$ be the first successful \nf{p.Refresh} that reads some value greater than ${b}$ for \nf{left.head} therefore contains ${B}$ in its created block in Line \ref{invokeCreateBlock}. Let the index of the block $R_p$ put in $n$\nf{.parent}\nf{.blocks} be $j$.
 
Since the index of the superblock of ${B}$ is not known until $B$ is propagated, $R_n$ cannot set the \nf{super} field of ${B}$ while creating it. One approach for $R_n$ is to set the \nf{super} field of ${B}$ after propagating $B$ to $n$\nf{.parent}. This solution would not be efficient because there might be $n$\nf{.parent} subblocks in the block $R_p$ propagated needing updates their \nf{super} field. However, intuitively, once $B$ is installed, its superblock is going to be close to \nf{$n$.parent.head} at the time of installation. If we know the approximate position of the superblock of $B$ then we can search for the real superblock when we it is needed. Thus, \nf{$B$.super} does not have to be the exact location of  the superblock of $B$, but we want it to be close to $j$. We can set \nf{$B$.super} to \nf{$n$.parent.head} while creating $B$, but the problem is that there might be many \nf{$n$\nf{.parent}.Refresh}es that could happen after $R_n$ reads $n$\nf{.parent}\nf{.head} and before propagating $B$ to $n$\nf{.parent}. If $R_n$ sets \nf{$B$.super} to \nf{$n$\nf{.parent}.head} after appending $B$ to \nf{$n$.blocks} (Line \ref{setSuper1}), $R_n$ might go to sleep at some time after installing $B$ and before setting \nf{$B$.super}. In this case, the next \nf{Refresh}es on $n$ and \nf{n.parent} help fill in the value of \nf{$B$.super}.

Block $B$ is appended to \nf{$n$.blocks[i]} on Line \ref{cas}. After appending $B$, \nf{$B$.super} is set on Line \ref{setSuper1} of a call to \nf{Advance} from \nf{$n$.Refresh} by the same process or another process or by Line \ref{helpAdvance} of a \nf{$n$.parent.Refresh}. We bound how far \nf{b.super} is from the index of $b$'s superblock by 1.
...
\State\tt{subBlock= right.\Call{BinarySearch}{sum\sub{enq}, i+blocks[b-1].sum\sub{enq-right}, blocks[b-1].end\sub{right}+1, blocks[b].end\sub{right}}} \label{rightChildGet}
...
We designed a tree to achieve agreement among processes in poly-log steps with respect to the total number of operations invoked and used it to implement a poly-log queue with respect to the size of the queue. We can use our solution for agreement to construct more poly-log wait-free linearizable objects. We call our solution \it{Block Tree}. In the next paragraphs we talk about possible improvements on block trees and the data structures that we can implement with block trees.

\paragraph{Reducing Space Usage}
Our queue algorithm's usage of memory has places to improve. We assumed arrays in the nodes are unbounded.  If there is an $N$, such that the $N$ is greater than the  maximum possible operations appended to the block tree, then we can use Array
Segment~\cite{7073592} model to take $O(n+ \log N)$ space in each node, where $n$ is the total number of operations. To implement this we can create a static array of pointers called \nf{arr} of references to array segments. When a process wishes to write into \nf{head} it checks whether \nf{arr[$\lfloor\log $ head$\rfloor$]} points to an array or not. If not, it creates a shared array with size $2^i$ and tries to \nf{CAS} a pointer to the created array into \nf{arr[$\lfloor\log $ head$\rfloor$]}. Whether the \nf{CAS} is successful or not \nf{arr[$\lfloor\log $ head$\rfloor$]} points to an array. When a process wishes to access to $i$th element it looks up to \nf{arr[$\lfloor\log i\rfloor$][$i-2^{\lfloor\log i\rfloor}$]} which takes $O(1)$ steps. Note that CAS Retry Problem does not happen here because if $n$ elements are appended to the array then only $O(p\times\log n)$ \nf{CAS} steps have happened. Furthermore at most $p$ arrays with size $2^{\lfloor\log i\rfloor}$ are allocated by processes while processes try to to the \nf{CAS} on \nf{arr[$i$]} which in average the time taken to allocate arrays in an execution is $O(1)$ per operation.
\begin{figure}[hbt]  
...
We can keep track of the \nf{block}s in the \nf{root} that all of their operations are terminated, i.e, all of its enqueues have been dequeued and the response of all of its dequeues is computed. We call these blocks \it{Finished Blocks}. If we help the old operations to compute their response, then we can say if block $B$ is finshed then all blcoks before $B$ are also finished. Knowing the most recent finished block we can release the memory taken by these blocks. To throw the garbage in the blocks away we cannot use arrays. We need a datastructure that supports \nf{read(i)}, \nf{write(i)} and \nf{split(i)} operations in $O(\log n)$. \nf{split(i)} is to remove all the indices less than \nf{i}. We can use Persistent Red Black Trees~\cite{DBLP:conf/podc/BashariW21} for this usage.
...
Assume a data structure storing a sequence that has supports three operations \nf{append(e)}, \nf{get(i)} and \nf{index(e)}. \nf{append(e)} adds \nf{e} to the sequence, \nf{get(i)} gets the \nf{i}th element in the sequence and \nf{index(e)} computes the position of appended element \nf{e} in the sequence. We can modify our queue to design such data structure.
If we do not distinct between \nf{Enqueue}s and \nf{Dequeue}s and replace \nf{DSearch} with Binary Search on all the \nf{root} in our algorithm, we achieve this with poly-logarithmic steps for each operation with respect to the number of \nf{append}s done.

\paragraph{Stacks}
There are two reasons block tree worked well to implement a queue. Firstly, to respond a \nf{Dequeue} we do not need to look at all the history of operations, if a \nf{Dequeue} does not return null we can compute the index of the \nf{Enqueue} that is response of a \nf{Dequeue} in $O(\log n)$ if we keep the number of enqueues and the size. Secondly, the operations needed to look to respond a the \nf{Dequeue} is not very far from it in the sequence of operations: the distance is at most linear in the size of the queue. It may be possible to create wait-free poly-logarithmic implementaion of other objects whose operations satisfy these two conditions. 
