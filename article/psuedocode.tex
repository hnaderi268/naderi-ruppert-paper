...
\begin{itemize}
%\item \tt{\sl{*Node} left(), right(), parent()} \textsf{: computed as mentioned in the \tt{Tree[]}}
...
\State \tt{tree[leaf].}\Call{Append}{newBlock}
\State \tt{<r\sub{deq}, b\sub{r\sub{deq}}>=} \tt{tree[leaf].}\Call{Index}{tree[leaf].head, 1} \cmt{\tt{r} is the rank among all dequeues of the dequeue and \tt{b\sub{r\sub{deq}}} is the index of the block in the root containing the dequeue.}
\State \tt{<r\sub{enq}, b\sub{r\sub{enq}}>=} \Call{FindResponse}{r\sub{deq}, b\sub{r\sub{deq}}} \cmt{\tt{r\sub{deq}} is the rank of the enqueue which is the response to the dequeue or -1 if the response is \tt{null}.} \label{deqRest}
\If{\tt{r\sub{enq}==-1}}
\State \tt{output= null}
...
\State \Return \tt{-1, 0}
\Else
\State \tt{res= root.blocks[b-1].sum\sub{deq}- root.blocks[b-1].size + i}
\State \Return \tt{<res, tree[0].blocks.get(enq, res).order>}
\EndIf
...
\State \tt{<new, n\sub{left}, n\sub{right}>= this.\Call{CreateBlock}{h}} \cmt{\tt{n\sub{left}, n\sub{right}} are the values read from the children's \tt{num\sub{propagated}}s.}
...
\State \tt{\Call{CAS}{dir.super[n\sub{dir}], null, h+1}}
\State \tt{\Call{CAS}{dir.num\sub{propagated}, n\sub{dir}, n\sub{dir}+1}}
\EndFor
...
\State \tt{block\sub{prev}= n.dir.blocks[index\sub{prev}]}
\State \cmt{\tt{newBlock} includes \tt{n.dir.blocks[index\sub{prev}+1..index\sub{last}]}.}
\State \tt{n\sub{dir}= n.dir.num\sub{propagated}}
\State \tt{newBlock.end\sub{dir}= index\sub{last}}
\State \tt{newBlock.num\sub{enq-dir}= block\sub{last}.sum\sub{enq} - block\sub{prev}.sum\sub{enq}}
\State \tt{newBlock.num\sub{deq-dir}= block\sub{last}.sum\sub{deq} - block\sub{prev}.sum\sub{deq}}
\State \tt{newBlock.sum\sub{enq-dir}= n.blocks[i-1].sum\sub{enq-dir} + b.num\sub{enq-dir}}
\State \tt{newBlock.sum\sub{deq-dir}= n.blocks[i-1].sum\sub{deq-dir} + b.num\sub{deq-dir}}
\EndFor
...
\Function{element}{Get}{\sl{int} b, \sl{int} i} 
\If{\tt{this {\keywordfont is} leaf}} \Return \tt{this.blocks[b].element}
\Else
\If{\tt{i $\leq$ this.blocks[b].num\sub{enq-left}}} \cmt{\tt{i} exists in the left child of this node}
\State \tt{subBlock= this.left\Call{BSearch}{sum\sub{enq}, i, this.blocks[b-1].end\sub{left}+1, this.blocks[b].end\sub{left}}}
\State \Return\tt{this.left.}\Call{Get}{i-this.left.blocks[subBlock-1].sum\sub{enq}, subBlock} 
\Else
\State \tt{i= i-this.blocks[b].num\sub{enq-left}}
\State\tt{subBlock= this.right\Call{BSearch}{sum\sub{enq}, i, this.blocks[b-1].end\sub{right}+1, this.blocks[b].end\sub{right}}}
\State \Return\tt{this.right.}\Call{Get}{i-this.right.blocks[subBlock-1].sum\sub{enq}, subBlock} 
\EndIf
\EndIf
\EndFunction{Get}

\Statex
\Statex $\leadsto$ \textsf{Precondition: \tt{b}th block of the node has propagated up to the root and \tt{i}th dequeue in the node is in \tt{this.blocks[b]}.}
\Function{<int, int>}{Index}{\sl{int} b, \sl{int} i} \cmt{Returns the order in the root of $i$th dequeue in the $b$th block of node n among dequeues.}
\If{\tt{this {\keywordfont is} root}} \Return \tt{<i, b>}
\Else
\State \tt{dir= (this.parent().left==n)? left: right} \cmt{check \tt{n} is a left or a right child}
\State \tt{superBlock= \Call{BSearch}{this.parent(), this.sum\sub{deq-dir}, i, super[this.blocks[b].group]-p, super[this.blocks[b].group]+p}} \cmt{superblock's group has at most $p$ difference with the value stored in \tt{super[]}.}
\If{\tt{dir {\keywordfont is} left}}
\State \tt{i+= this.parent().blocks[superBlock-1].sum\sub{deq-right}}
\Else \State \tt{i+= this.parent.blocks[superBlock-1].sum\sub{deq} + this.blocks[superBlock].sum\sub{deq-left}} \cmt{consider dequeues from the right child}
\EndIf
\State \Return\Call{Index}{this.parent(), superBlock, i}
\EndIf
\EndFunction{Index}
...
\State \tt{b.group= head} \label{appendStart} \cmt{Append is only called by the owener of the leaf.}
