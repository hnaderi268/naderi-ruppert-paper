...
\nf{n.head} is non-decreasing over time and \nf{n.blocks[i].end\sub{left, right}} is non-decreasing over \nf{i}.
\end{lemma}
\begin{proof}
...
\begin{lemma}[headPosition] \label{lem::headPosition} If the value read in Line \ref{prevLine}(\nf{h=n.head}) is $h$ then, \nf{n.blocks[i]=null} for \nf{i>h} and \nf{n.blocks[i]$\neq$null} for \nf{i<h}.
\end{lemma}
\begin{proof}
...
\begin{figure}[hbt]
  \center\includegraphics[width=6in]{pics/completedouble.png}
  \caption{$t_t$ is after $R_2^\prime$'s reads n.head after n.head is incremented which is after the succesufil TryAppend(i) which is after $R_1$'s reading n.head that takes place at $t_i$.}
\end{figure}
