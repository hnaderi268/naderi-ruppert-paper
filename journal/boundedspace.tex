...
polynomial in $p$ and $q$, while ensuring the (amortized) step complexity is still polylogarithmic \rev{in $p$ and $q$}. We replace the \fld{blocks} array in each node by a red-black tree (RBT)
...
This \rev{increases the number of steps taken by any routine designed for a (sequential) RBT only by a constant factor, assuming that nodes are accessed only by following pointers from the root (as is the case for all RBT operations we require)}.
...
also keep the \rev{step complexity} of our operations polylogarithmic \rev{in $p$ and $q$}.
...
To maintain good amortized \rev{step complexity}, we periodically do a garbage collection (GC) phase.
...
these obsolete blocks from an RBT using \rev{a logarithmic number of steps} \cite[Sec.~4.2]{Tar83}.
