...
Thus, it is lock-free but not wait-free and has an amortized \here{Maybe we can say worst-case. I see that you have used amortized step complexity consistently everywhere but maybe it is fair to say that these papers were not designed to improve the worst-case amortized step complexity.} step complexity
...
In both cases, the helping mechanisms only add to the step complexity of the MS-queue. \here{I get what this sentence means, but from a high overview i think it is inaccurate. Since the step complexity of MS-queue is infinite, so nothing can be added to it and a wait-free queue is always better than a lock-free queue in the worst-case.}
...
\here{Is it ok for a reader to talk about A and then start a sentence about similarity to B? My suggestion which is longer: Kogan and Petrank~\cite{10.1145/2370036.2145835} proposed the fast-path slow-path methodology which is similar to Morrison and Afek \cite{DBLP:conf/ppopp/MorrisonA13}. Other queues \cite{Nik19,10.1145/3490148.3538572,DBLP:conf/ppopp/YangM16} have used the methodology.} Similarly,
