...
\State \Return \tt{<root.\Call{DSearch}{r\sub{enq}, b}, r\sub{enq}>}
\EndIf
...
\Function{int}{DSearch}{\sl{int} i, \sl{int} end}
...
\State \tt{start= start-(b-start)}
\EndWhile
\Return \tt{root.BSearch(sum\sub{enq}, r\sub{enq}, start, b)}
\EndFunction{DSearch}
...
\State \tt{newBlock.length= max(root.blocks[i-1].length + b.num\sub{enq} - b.num\sub{deq}, 0)}\label{computeLength}
...
The set of operations of block $b$ are the operations in the leaf sub blocks of $b$. We show the set of operations of block by \nf{ops(b)}. We also say $b$ contains $op$ if $op\in ops(b)$.
\end{definition}
...
\begin{lemma}[Computing Queue's Head block] \label{computeHead}
  ...
  \end{lemma}
\begin{proof}
  The length of the queue if the operations in the prefix for the $b$th block in the root are applied with the order of $L$ is stored in the \nf{root.blocks[b].length}. It is computed while creating the block in Line~\ref{computeLength}. If the length of a queue is greater than 0 then a \nf{Dequeue()} would decrease the length of the queue, otherwise the length of the queue remains 0. Having length of the queue after the previous block and number of enqueues and dequeues in the block, Line~\ref{computeLength} computes wether the queue becomes empty or the length of it.
\end{proof}
\framebox[1.1\width]{HOW?} How to prove mathematicaly that \nf{ax(root.blocks[i-1].length + b.num\sub{enq} - b.num\sub{deq}, 0)} is the length of the queue after the block. I can only explain it here.
%\begin{proof}
