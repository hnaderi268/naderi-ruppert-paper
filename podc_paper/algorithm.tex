...
\begin{figure}
\begin{minipage}[t]{0.461\textwidth}
\begin{algorithmic}[1]
...
\end{algorithmic}
\end{minipage}
\begin{minipage}[t]{0.533\textwidth}
...
\Function{Block}{CreateBlock}{\var{Node v, int i}} 
	...
	\State \hangbox{\var{new.sum\sub{enq}} \assign\ \var{v.left.blocks[new.\eleft].sum\sub{enq}} + \\
			\var{v.right.blocks[new.\eright].sum\sub{enq}}}
	\State \hangbox{\var{new.sum\sub{deq}} \assign\ \var{v.left.blocks[new.\eleft].sum\sub{deq}} + \\
			\var{v.right.blocks[new.\eright].sum\sub{deq}}}
...
\Function{<int, int>}{IndexDequeue}{\var{Node v, int b, int i}}
    ...
    \Else
	    ...
	    \State \hangbox{\var{i += v.blocks[b-1].sum\sub{deq}} - \\
	    		\var{v.blocks[v.parent.blocks[sup-1].end\sub{dir}].sum\sub{deq}}}
        \If{$\var{dir} = \var{right}$} 
        	\State \hangbox{\var{i += v.blocks[v.parent.blocks[sup].end\sub{left}].sum\sub{deq}} -\\
					\var{v.blocks[v.parent.blocks[sup-1].end\sub{left}].sum\sub{deq}}\label{considerLeftBeforeRight}}
...
\Function{element}{FindResponse}{\var{int b, int i}}
    ...
    \If{\var{root.blocks[b-1].\size + root.blocks[b].num\sub{enq}} $<$ \var{i}}\label{checkEmpty}
        \State \Return \var{null} \hfill \com\ queue is empty when dequeue occurs\label{returnNull}
    \Else \ \linecomment{response is the \var{e}th enqueue in the root}
        \State $\var{e} \assign\ \var{i} + \var{root.blocks[b-1].sum\sub{enq}} - 
	\var{root.blocks[b-1].size}$\label{computeE}
		\State \linecomment{compute enqueue's block using binary search}
		\State find min $\var{b}'\leq \var{b}$ with $\var{root.blocks}[\var{b}'].\var{sum\sub{enq}} \geq \var{e}$
		\State \linecomment{find rank of enqueue within its block}
		\State $\var{i}' \assign\ \var{e} - \var{root.blocks}[\var{b}'-1].\var{sum\sub{enq}}$
        \State \Return \Call{GetEnqueue}{root, $\var{b}'$, $\var{i}'$}\label{findAnswer}
    \EndIf
...
\Function{element}{GetEnqueue}{\var{Node v, int b, int i}} \Comment{returns argument of ith enqueue in $E(\var{v.blocks[b]})$}
    \State \linecomment{Precondition: $\var{i}\geq 1$ and $E(\var{v.blocks[b]})$ contains at least \var{i} enqueues}

    \If{\var{v} is a leaf node}
        \State\Return \var{blocks[b].element} \label{getBaseCase}
    \Else 
        \State \var{sum\sub{left}} \assign\ \var{v.left.blocks[v.blocks[b].\eleft].sum\sub{enq}} \Comment{\#\ of enqueues in \var{v.blocks}[1..$\var{b}$] from left child}
        \State \var{prev\sub{left}} \assign\ \var{v.left.blocks[v.blocks[b-1].\eleft].sum\sub{enq}} \Comment{\#\ of enqueues in \var{v.blocks}[1..$\var{b}-1$] from left child}
        \State \var{prev\sub{right}} \assign\ \var{v.right.blocks[v.blocks[b-1].\eright].sum\sub{enq}} \Comment{\#\ of enqueues in \var{v.blocks}[1..$\var{b}-1$] from right child}
        \If{$\var{i} \leq \var{sum\sub{left}} - \var{prev\sub{left}}$} \label{leftOrRight} \cmt{required enqueue is in \var{v.left}}
        	\State \linecomment{Find enqueue's block in \var{v.left} and its rank within block}
            \State find min $\var{b}'$ in range [\var{v.blocks[b-1].\eleft}+1..\var{v.blocks[b].\eleft}] s.t. $\var{v.left.blocks}[\var{b}'].\var{sum\sub{enq}} \geq \var{i} + \var{prev\sub{left}}$
            \State $\var{i}'$ \assign\ $\var{i} - (\var{v.left.blocks}[\var{b}'-1].\var{sum\sub{enq}} - \var{prev\sub{left}})$
            \State \Return\Call{GetEnqueue}{\var{v.left}, $\var{b}'$, $\var{i}'$} 
        \Else \Comment{required enqueue is in \var{v.right}}
            \State $\var{i}\ \assign\ \var{i} - (\var{sum\sub{left}} - \var{prev\sub{left}})$
        	\State \linecomment{Find enqueue's block in \var{v.right} and its rank within block}
            \State find min $\var{b}'$ in range [\var{v.blocks[b-1].\eright}+1..\var{v.blocks[b].\eright}] s.t. $\var{v.right.blocks}[\var{b}'].\var{sum\sub{enq}} \geq \var{i} + \var{prev\sub{right}}$
            \State $\var{i}'$ \assign\ $\var{i} - (\var{v.right.blocks}[\var{b}'-1].\var{sum\sub{enq}} - \var{prev\sub{right}})$
            \State \Return\Call{GetEnqueue}{\var{v.right}, $\var{b}'$, $\var{i}'$} 
        \EndIf
    \EndIf
\EndFunction{GetEnqueue}
\end{algorithmic}
\caption{\label{pseudocode2}Queue implementation, continued. \here{Would this routine be simpler to write if i parameter be the rank within whole node rather than within the block?}}
\end{figure}
