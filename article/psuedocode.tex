...
\Statex $\blacktriangleright$ \tt{\sl{Block b}} \Comment{If \tt{b} is \tt{blocks[i](i!=0)} then \tt{b[-1] is blocks[i-1].}}
...
%  \textsf{: total \# operations in block, prefix sum of \tt{num}}
  ...
  \item \tt{\sl{int} group}
  \textsf{: the value read from the node \tt{n.counter} when appending this block to \tt{n}.}
\end{itemize}
...
\State \tt{block newBlock= \Call{new}{\sl{LeafBlock}}} \cmt{Creates a null element block, appends it to the tree, computes its order among operations, then computes its response index if it exists and returns the response's element.}
...
\State \tt{tree[leaf].}\Call{Append}{newBlock}
\State \tt{<i, b\sub{i}>=} \tt{tree[leaf].}\Call{Index}{tree[leaf].head, 1} \cmt{\tt{i} is the rank among all dequeues of the dequeue in the root and \tt{b\sub{i}} is the index of the block in the root containing it.}
\State \tt{index\sub{response}=} \Call{ComputeDeqRes}{i, b\sub{i}} \cmt{\tt{index\sub{response}} is the index of the enqueue which is the response to the dequeue or -1 if the response is \tt{null}.} \label{deqRest}
\If{\tt{index\sub{response}==-1}}
\State \tt{output= null}
\State \tt{tree[0].blocks[b\sub{i}].num\sub{finished}.inc()} \cmt{shared counter}
\If{\tt{tree[0].blocks[b\sub{i}].num\sub{finished}==tree[0].blocks[b\sub{i}].num}}\cmt{all the operations in the block containing the dequeue are finished.}
\State \tt{tree[leaf].last\sub{done}= b\sub{i}}
\EndIf
\Else
\State \tt{output= \Call{Get}{index\sub{response}}}
\State \tt{b\sub{r}= tree[0].blocks.get(enq, index\sub{response}).order}\cmt{index of the block in the root that contains response enqueue.}
\State \tt{root.blocks[b\sub{r}].num\sub{finished}.inc()}
\State \tt{root.blocks[b\sub{r}].num\sub{finished}.inc()}
\If{\tt{root.blocks[b\sub{r}].num\sub{finished}==root.blocks[b\sub{r}].num}}\cmt{become done}
\State \tt{tree[leaf].last\sub{done}= b\sub{r}}
\ElsIf {\tt{root.blocks[b\sub{r}].num\sub{finished}==root.blocks[b\sub{r}].num}} \cmt{\tt{root.blocks[b\sub{r}]} comes after \tt{root.blocks[b\sub{i}]}.}
...
\If{\tt{not} this.\Call{Refresh}{}}
\State this.\Call{Refresh}{this} \cmt{Lemma Double Refresh}
\EndIf
...
\ElsIf{\tt{root.blocks.tryAppend(new)}} \cmt{\it{how to put space in he first of the new line?}}
\ForEach{\tt{dir} {\keywordfont{in}} \tt{\{left, right\}}} \label{okcas}
\State \tt{\Call{CAS}{dir.super[c\sub{dir}], null, h+1}} \cmt{Superblock's Lemma}
\State \tt{\Call{CAS}{dir.counter, c\sub{dir}, c\sub{dir}+1}}
\EndFor
