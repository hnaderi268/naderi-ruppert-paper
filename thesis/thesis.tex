...
$R_n$ puts $B$ into $n$\nf{.blocks[}$b$\nf{]} so ${R_n}$ reads value $b$ for $n.$\nf{head}. Since $R_{p2}$'s \nf{CAS} into $n$\nf{parent.blocks} is successful there should be a \nf{Refresh} instance $R_p^\prime$ on $n$\nf{.parent} that increments $n$\nf{.parent} after $R_{p1}$'s \ref{cas} and before $R_{p2}$'s \ref{readHead}. We assumed $t^{R_n}_{\ref{cas}}<t^{R_{p1}}_{\ref{cas}}<t^{R_{p2}}_{\ref{cas}}$. Finally, since \ref{readChildHead} is after \ref{readHead} and $R_{p2}$'s \ref{readHead} is after $R_p^\prime$'s \ref{incrementHead}, $R_{p2}$ reads a value $\geq b$ for $n.\nf{head}$.

$$_{\ref{cas}}^{R_n}t<_{\ref{cas}}^{R_p1}<_{\ref{incrementHead}}^{R_p^\prime}t<_{\ref{cas}}^{R_p2}t<_{\ref{readChildHead}}^{R_p2}$$
TODO: draw figure
...
Furthermore, if $B$\nf{.super} is not set earlier it is set by $R_{p2}$ in Line \ref{setSuper1} invoked from \ref{helpAdvance}.
