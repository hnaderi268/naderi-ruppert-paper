...
\here{?  They also showed that a randomized implementation of the ordering tree technique can
be used to obtain a (doubly-ended) queue with O(\here{?}) amortized expected step complexity,
beating the  $\Omega(\min(c,\log\log p))$ lower bound on deterministic algorithms.}
