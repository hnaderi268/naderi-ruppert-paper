...
using atomic access to $\Omega(p \log p)$-bit words. 
(Thus, in terms of operations on reasonably-sized words, their construction would still take $\Omega(p)$ steps per operation.)
...
Ellen and Woelfel introduced an implementation of a Fetch\&Increment object
...
logarithmic complexity is to use a tree storing the Fetch\&Increment
operations invoked by processes. When a process wants to do a
Fetch\&Increment it adds its Fetch\&Increment to the tree and returns the number
of elements in the tree. There are some similarities between designing
a queue and a Fetch\&Increment object. A Fetch\&Increment object can be
constructed from a queue. The algorithm by Ellen and Woelfel is
interesting because of the similarities between Fetch\&Increment objects and
