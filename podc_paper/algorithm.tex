...
\here{This parag may need to be rewritten if we use sum\sub{enq} field instead of sum\sub{enq-left} and sum\sub{enq-right}.  Also, maybe it is too detailed at this point; perhaps instead just say that for task \ref{findenqueue} we want to be able to find the position of the $r$th enqueue in a block within a block in its child using binary search.}
...
\var{sum\sub{enq-left}} and \var{sum\sub{enq-right}} fields.
\here{modify to \var{sum\sub{enq}} and \eleft/\eright.}
...
For this, each block in the root stores a \var{size} field that stores the number of elements
