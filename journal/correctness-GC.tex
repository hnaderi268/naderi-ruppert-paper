...
the \op{Refresh} then creates the  block \var{new},
...
is also finished when $st$ occurs.  This completes the proof of \Cref{splitfinished}.
...
Moreover, by \Cref{lem::finished}, if the operation is a dequeue, then its result is in its \fld{response} field.
...
It follows from \Cref{RBTbound} and our choice of $G=p^2\ceil{\log p}$
