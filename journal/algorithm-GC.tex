...
\gc{
\Function{Object}{CompleteDeq}{\var{leaf}, $h$} \Comment{finish propagated dequeue in $\var{leaf}.\fld{blocks}[h]$}
...
\end{algorithmic}
\caption{Bounded-space queue implementation's main routines.\label{code1GC}}
\end{figure*}

\begin{figure*}
\begin{algorithmic}[1]
\setcounter{ALG@line}{247}
\gc{
...
\Function{Block}{CreateBlock}{\typ{Node} \var{v}, \typ{int} \var{i}} \linecomment{create new block  to install in \var{v}.\fld{blocks}[\var{i}]}
    \State \hangbox{let \var{new} be a new \typ{Block} with fields\label{initNewBlockGC}
    \gc{
	...
	\fld{sum\sub{enq}} \assign\ \var{v}.\fld{left.blocks}[\var{new}.\eleft].\fld{sum\sub{enq}} + 
			\var{v}.\fld{right.blocks}[\var{new}.\eright].\fld{sum\sub{enq}}\\
	\fld{sum\sub{deq}} \assign\ \var{v}.\fld{left.blocks}[\var{new}.\eleft].\fld{sum\sub{deq}} +
    ...
    \If{$\var{v} = \var{root}$} \var{new}.\fld{size} \assign\ max(0, $\var{v}.\fld{blocks}[\var{i}-1].\size\ + 
        	\var{num\sub{enq}} - \var{num\sub{deq}}$)\label{computeLengthGC}
    \EndIf
    \If{$\var{num\sub{enq}} + \var{num\sub{deq}} = 0$} \Return \nl \hfill \Comment{no blocks need to be propagated to \var{v}}\label{testEmptyGC}\label{CBnullGC}
    \Else\ \Return \var{new}
    \EndIf
