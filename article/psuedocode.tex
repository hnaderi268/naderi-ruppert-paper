...
\begin{itemize}
\item Prefix of the $op$ in the sequence of operations $S$, is the operations strictly before $op$.
  \item $E(n,b)$ is the sequence of enqueue operations in \nf{ops(n.blocks[b])}. $E(leaf,b)$ is the enqueue operation in \nf{ops(leaf.blocks[b])}.
\begin{align*} 
E(n,b) =&  E(n.left,n.blocks[b-1].end\sub{left}+1).E(n.left,n.blocks[b-1].end\sub{left}+2)...E(n.left,n.blocks[b].end\sub{left}). \\ 
&E(n.right,n.blocks[b-1].end\sub{right}+1).E(n.right,n.blocks[b-1].end\sub{right}+2)...E(n.right,n.blocks[b].end\sub{right})
\end{align*}
  \item $E_{n,b,i}$ is the $i$th enqueue in $E(n,b)$.
\item Order of the enqueue operations in the node $n$: $E(n)=E(n,1).E(n,2).E(n,3)...$
\item $E_{n,i}$ is the $i$th enqueue in $E(n)$.
  \item $D(n,b)$ is the sequence of dequeue operations in \nf{ops(n.blocks[b])}. $D(leaf,b)$ is the dequeue operation in \nf{ops(leaf.blocks[b])}.
\begin{align*} 
D(n,b) =&  D(n.left,n.blocks[b-1].end\sub{left}+1).D(n.left,n.blocks[b-1].end\sub{left}+2)...D(n.left,n.blocks[b].end\sub{left}). \\ 
&D(n.right,n.blocks[b-1].end\sub{right}+1).D(n.right,n.blocks[b-1].end\sub{right}+2)...D(n.right,n.blocks[b].end\sub{right})
\end{align*}
    \item $D_{n,b,i}$ is the $i$th enqueue in $D(n,b)$.
\item Order of the dequeue operations in the node $n$: $D(n)=D(n,1).D(n,2).D(n,3)...$
\item $D_{n,i}$ is the $i$th dequeue in $D(n)$.
\item Linearization: $L=E(root,1).D(root,1).E(root,2).D(root,2).E(root,3).D(root,3)...$
\end{itemize}
\end{definition}
\it{Note that in the non-root nodes we only need ordering of enqueues and dequeues among the operations of their own type. Since \nf{GetENQ()} only searches among enqueues and \nf{IndexDEQ()} works on dequeues.}
...
  Consider two consecutive instances $R_1$, $R_2$ of \nf{Refresh()} on \nf{n} by process $p$. Let $t_1$ be the time $R_1$ is invoked and $t_2$ be the time $R_2$ terminated. If $R_1$ and $R_2$ both fail and return false, then we have \nf{ops(EST\sub{n.left, t\sub{1}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{1}})} $\subseteq$ \nf{ops(EST\sub{n, t\sub{2}})}.
\end{lemma}
\begin{proof}
  
...
\begin{definition}
  $t_{before \; line}$ is the immediate time before running Line $line$.   $t_{before \; line}$ is the immediate time after running Line $line$.
\end{definition} 
\begin{corollary}
  \nf{ops(EST\sub{n.left, t\sub{before \ref{firstRefresh}}})} $\cup$ \nf{ops(EST\sub{n.right, t\sub{before \ref{firstRefresh}}})} $\subseteq$ \nf{ops(EST\sub{n, t\sub{after \ref{secondRefresh}})}}
\end{corollary}
\begin{proof}
...
\end{lemma}
\begin{proof}
To derive a contradiction, assume there are 2 operations $op_1$ and $op_2$ from process $p$ in block $b$ in node $n$. WLOG $op_1$ is invoked earlier than $op_2$. A process cannot invoke more than one operations concurrently, so one operation has to be finished before the other starts. So $op_1$ has terminated before $op_2$ started. By Corollary \ref{lem::appendExactlyOnce} before appending $op_2$ to the tree $op_1$ exists in every node from the path of $p$'s leaf to the root. Because $op_1$'s \nf{Append} is finished before $op_2$'s \nf{Append} starts. So there is some block $b^\prime$  before $b$ in $n$ containing $op_1$. $op_1$ existing in $b$ an $b^\prime$ contradicts Lemma \ref{append}.
\end{proof}
...
\end{lemma}
\begin{proof}
  It follows directly from Lemma \ref{blockSize} and the observation that each block appended to the tree contains at least one operation, induced from Line~\ref{addOP}. 
\end{proof}
...
Hello Im hoseen master student in cs at York uni. I will defend my thesis by Nov this year and I was looking for ful time jobs starting from novomber. I have experince in both acdemia and indusrty. When I was a bachelors srudent I workd from 2 years as a Software dveloper. I have expereince with backend devlopment. I have implemented, deploeyd an maintained backend systems.
