...
Accessing the block in entry $i$ in the \fld{blocks} array is 
...
If a \op{Refresh} on a node adds a block whose \var{index} is a multiple of $G=(p\ceil{\log p})^2$, it does GC to remove obsolete blocks from the node's RBT.
