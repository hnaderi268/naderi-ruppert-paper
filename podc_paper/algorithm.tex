...
\Function{void}{Advance}{\typ{Node} \var{v}, \typ{int} \var{h}} \com\ set \var{v}.\fld{blocks}[\var{h}].\fld{super} and increment
    \State \linecomment{\var{v}.\fld{head} from \var{h} to $\var{h}+1$}
    \If{$\var{v}\neq \var{root}$}
...
\Function{Block}{CreateBlock}{\typ{Node} \var{v}, \typ{int} \var{i}} \com\ create new block for a \op{Refresh} to install in \var{v}.\fld{blocks}[\var{i}]
...
\end{algorithmic}
\caption{\label{pseudocode2}Queue implementation, continued.\Eric{Fix code and combine with other figure}}
