...
Linearizability and lock-freedom are standard requirements for designing shared data structures. To the best of our knowledge, all of the existing linearizable lock-free queues in the literature have a common problem in their worst case, the CAS Retry Problem. We show that our algorithm avoids this problem and has a worst-case running time better than prior lock-free queues. The amortized number of steps for an \nf{Enqueue} or \nf{Dequeue} in our algorithm is $O(\log^2 p + \log q)$,  where $p$ is the number of processes and $q$ is the size of the queue when the operation is linearized.
\end{abstract}
...
\begin{figure}[hbt]
  ...
  \caption{\label{fig::badexample}An example of an execution on an empty queue that is not linearizable. \texttt{Enqueue(A)} has terminated before \texttt{Enqueue(B)} is invoked and the \texttt{Dequeue} can occur before termination of \texttt{Enqueue(A)} and return \nf{null} or can occur after termination of \texttt{Enqueue(A)} and return \texttt{A}.}
\end{figure}
...
\paragraph{Thesis outline} The rest of the thesis is organized as follows. Chapter 2 gives an outline of related work done in the area and the motivation for our implementation. Previous lock-free queues and their common problem is presented in Section 2.1. In Section 2.2, we mention some restricted lock-free queues. Section 2.3 talks about poly-logarithmic constructions of shared objects. Section 2.4 ends with a lower bound on the amortized time complexity of shared queues.

In Chapter 3 we introduce a poly-logarithmic step wait-free queue. In Section 3.1 we give a high-level description of our implementation. We also discuss the motivation and requirements in our design to achieve poly-log time. Section 3.2 contains the algorithm itself.
...
\begin{figure}[hbt]
  ...
  \caption{Baskets queue idea. At a time, all operations in a basket ran concurrently, but only one succeeded to do \texttt{CAS}. To order the operations in a basket, the mechanism in the process algorithm is to \texttt{CAS} again. The successful process will be the next one in the basket, and so on.}
\end{figure}
% \bf{TODO: make small circles the same size}
...
\ElsIf{\tt{i <= blocks[b].num\sub{enq-left}}} \label{leftOrRight} \cmt{$E_i(this,b)$ is in the left child of this node.}
...
Here we want to demonstrate how four processes help each other to  propagate their operations up to the root as time goes on and then compute their responses.

In Figure \ref{fig::exec1}, processes $P1,P2,P4$ have appended the blocks containing $ENQ_1,DEQ_2,DEQ_4$. Line \ref{appendLeaf} appends block $B$ to the leaf.

Then as in Figure \ref{fig::exec2}, $P2$ does a successful \nf{Refresh} on the node $n5$ and propagates both $P1$ and $P2$ operations. $P4$ does also a successful \nf{Refresh} on the node $n6$ and propagates $DEQ_4$. $P1$'s \nf{Refresh} on $n5$ is slower than $P2$'s and creates its new block after the $P2$'s successful \nf{CAS} that put $ENQ_1,ENQ_2$. So the block created by $P1$ is empty as there is nothing to propagate ($P1$'s \ref{readHead} is after $P2$'s \ref{cas}).

Since the block $P1$ creates is empty, it returns \nf{true} in Line \ref{addOP} as shown in Figure \ref{fig::exec3}. $P3$ propagates $ENQ_3$ to $n6$ and $P4$ is very fast and it reaches $DEQ_4$ to the \nf{root} even before $P2$ puts its block to $n5$.

Finally, $P4$ goes to sleep, and $P2$ helps to increment \nf{root.head} after its failed \nf{CAS} on the root. After incrementing \nf{root.head}, $P3$ succeeds to do \nf{root.Refresh} with a single \nf{CAS} on the \nf{root} and puts operations from $P1,P2$ and $P3$ to the \nf{root}. $P1$ fails to do \nf{Refresh} on the \nf{root} two times, but for sure, its operation is propagated after the second failed \nf{Refresh}. See Figure \ref{fig::exec4}

All the fields of some blocks from Figure \ref{fig::exec4} are filled in Figure \ref{fig::exec5}. $P1$ and $P3$'s operations terminated after that they appended the \nf{Enqueue}s to the root. $P2$ needs to compute the index of the block that its \nf{Dequeue} is in. It computes the superblock of its leaf block, which is the second block in $n5$. Then computes the superblock of $ENQ_1,DEQ_2$. As we can see\nf{super} field of  $ENQ_1,DEQ_2$ is 1 but the index of its superblock is 2. Finally reaches the block $ENQ_1,ENQ_3,DEQ_2$. The queue size after the previous block is 0 so $DEQ_2$ 's response is $ENQ_1$'s element. Then it tries to get the $ENQ_1$'s element. $ENQ_1$ is in the left subblock of  block $ENQ_1,ENQ_3,DEQ_2$; because $1<ENQ_1,ENQ_3,DEQ_2$\nf{.sum\sub{enq-left}}. It reaches the leaf $n1$ and returns $ENQ_1$'s element. $DEQ_4$ returns its response the same.

\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec1.png}
  \caption{$P1,P2,P4$ append operations. \label{fig::exec1}}
\end{figure}
...
\begin{figure}
  ...
  \caption{$P4$ puts its block to the \nf{root} before $P2$ appending its block to $n5$.\label{fig::exec3}}
\end{figure}

\begin{figure}
  \center\includegraphics[width=4.8in]{thesis/pics/exec4.png}
  \caption{$P3$ succeeds to the \nf{CAS} to the \nf{root}. $P1$'s work is done after two failed \nf{CAS}es. \label{fig::exec4}}
\end{figure}
...
\begin{figure}[hbt]
  ...
  \caption{Order of operations in b. Operations in the leaves are ordered in the numerical order shown in the drawing.}
\end{figure}
...
If $R$ is a complete instance of \nf{$n$.Refresh}, then we have $EST_{n\nf{.left}}^{t^R} \; \cup \; EST_{n\nf{.right}}^{t^R} \subseteq EST_{n}^{^Rt}$.
\end{corollary}
\begin{proof}
...
\subsection{Attiya--Fouren Lower Bound}
As we talked about earlier in Section 2.4 the Attiya--Fouren lower bound says that a concurrent implementation of queues using reads, writes and conditional operations like \nf{CAS} has $O(\sqrt{\log \log p})$ amortized complexity \cite{DBLP:conf/opodis/AttiyaF17} when the number of concurrent processes is $O(\log\log p)$. Our amortized complexity is $O(log^2p+log q)$. It is an open problem to reduce the gap between our algorithm and the Attiya--Fouren lower bound.
