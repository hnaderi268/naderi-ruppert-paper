...
\State \tt{CAS(dir.blocks[h\sub{dir}], null, h)}
\EndFor \label{endHelpChild1}
%\If{\tt{n.blocks[h]!=null?}} \tt{h+=1}\EndIf
\State \tt{new= \Call{CreateBlock}{h}} \label{invokeCreateBlock}
\If{\tt{new.num==0}} \Return{\tt{true}} \label{addOP} \cmt{Created block contains nothing.}
\ElsIf{\tt{blocks.tryAppend(new, h)}} \label{cas}
\ForEach{\tt{dir} {\keywordfont{in}} \tt{\{left, right\}}} \label{okcas}
\State \tt{hp= parent.\head} \label{readParentHead2}
\State \tt{\Call{CAS}{new.super, null, hp}} \label{setSuper}
...
\begin{lemma}
 Let the value When an instance of \nf{Refresh()} is called on the node \nf{n}, the instance is supposed to do a \nf{CAS} into \nf{n.blocks[h]} where \nf{h} is the value read from \nf{n.head}.
\end{lemma}
\begin{proof}
  By Invariant \ref{lem::headPosition}, \nf{n.blocks[h]} may be \nf{null} or not \nf{null}. This also holds for \nf{n.parent.blocks[hp]}. Unless all other fields of a block that is set  while creating the block because of a technical problem we set the \nf{super} field of a block after appending it to the blocklist. \nf{super} field is supposed to show the approximate index of the superblock of a block, We set the value while appending the block or while a block is appended to the parent. These times the value of parent's \nf{head} is really close to the actual value of the superblock of the block.
  
The \nf{Refresh} routine first helps to fill the \nf{super} field of \nf{n.blocks[h]} in case \nf{n.blocks[h]} is not null (Line \ref{helpSuper1}) and \nf{n.blocks[h].super} field is not set yet, and then the \nf{TryAppend} in Line \ref{cas} fails  and the \nf{Refresh} routine helps to increment \nf{n.head} value in Line \ref{incrementHead2}.

After helping to fill the current \nf{block} at the \nf{head} index, \nf{Refresh} helps the left child and the right child to fill their \nf{super} field. A process may go to sleeps after Line \ref{cas} of its \nf{Refresh} and before Line \ref{setSuper}. So when a block is going to be appended to parent we help the children so the super field dos not change very much form its real value. After this step we are assured for all the blocks in the children that their index is less than the \nf{head}, the \nf{super} field is set.

We claim after Line \ref{endHelpChild1} of \nf{n.Refresh()}, \nf{super} field of all the established blocks in \nf{n,n.left,n.right} is set and $\neq \nf{null}$.
\end{proof}
...
\begin{corollary}
  After a successful \nf{TryAppend(new,h)}, \nf{n.blocks[h].super} is set by itself or the process that incremented the head from \nf{h} to \nf{h+1}, or the process that did the Lines 313 \ref{startHelpChild1} to 316 \ref{endHelpChild1}.
\end{corollary}

\it{How I can make this proof coming precise?}
\begin{lemma}\label{superRange} If $\nf{n.blocks[b].super} \neq \nf{null}$ then, the superblock of \nf{n.blocks[b]} is in \nf{n.parent.blocks[b.super-1..b.super+1]}.
\end{lemma}
\begin{proof}
  From the previous corollary, \nf{n.blocks[h].super} might be set while \nf{n.blocks[b]} is appended which is going to be one place behind the real super block, or it might set after appending which means one block behind the real value. Let us explain the possible cases on block \nf{b} in node \nf{n}.

\nf{super} of the block field is set by the same process that appended the block. There are two possibilities that \nf{n.parent[hp]} is \nf{null} or not which \nf{hp} is the value of \nf{n.parent.head}/ In this case that \nf{super} field is the parent's \nf{head=hp} and when a block is going to be appended to the the \nf{hp+2} index of the parent it is going to contain the \nf{b}. One may wonder why, becuase it has read the parent's \nf{head} value \nf{hp+2} which is greater than \nf{hp+1} and it has to be incremented after \nf{n.bocks.TryAppend(new,h)}.

The \nf{super} field is set by some process doing a \nf{Refresh} on \nf{n} after appending \nf{b}. In this case also the \nf{super} value is going to be behind the index of the real superblock index in parent if the \nf{n.parent.blocks[hp]}  is already not \nf{null} and so the next of the next block appended to the parent is going to absolutely contain \nf{b} if the next block appending to \nf{n.parent} did not contain \nf{b} (Lemma \ref{trueRefresh}). One might wonder why the next block appended to the parent might not contain \nf{b} because it may read the blocks it was going to append before appending \nf{b} to \nf{n.blocks}.
