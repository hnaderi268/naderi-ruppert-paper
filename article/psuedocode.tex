...
Let \nf{R=n.Refresh} be an instance of \nf{Refresh} that reaches Line \ref{cas}. After \nf{R} terminates \nf{n.head} is greater than h, the value read  in line \ref{readHead} of \nf{R}.
\end{lemma}
\begin{proof}
If Line \ref{incrementHead1} or \ref{incrementHead1} are successful then the claim holds, otherwise another process has incremented the head from \nf{h} to \nf{h+1}.
\end{proof}
...
\end{corollary}
\begin{proof}
If \ref{firstRefresh} or \ref{secondRefresh} succeed, the claim is true by \ref{lem::trueRefresh}. Otherwise Lines \ref{firstRefresh} and \ref{secondRefresh} satisfy the preconditions of Lemma \ref{doubleRefresh}.
\end{proof}
...
\end{corollary}
\begin{proof}
After \nf{Append(blk)}'s termination, \nf{blk} is in \nf{root.blocks} since \nf{blk} is established  in the leaf it has been added to. By induction on \ref{doublyRefresh} it is propagated up to the root. Finally \ref{append} shows  only one block in the root contains \nf{blk}.
\end{proof}
...
\end{lemma}
\begin{proof}
By proof of contradiction, assume there are more than one operation from process $p$ in block $b$ in node $n$. A process cannot invoke more than one operations concurrently, so one operation hasto be finished before the other starts. From $p$ 's operations in $b$, let $op_1$ be the operation invoked first, before $op_2$. Note that $op_1$ has terminated before $op_2$ started. So  by Corollary 17 before appending $op_2$ to the tree $op_1$ exists in every node from the path of $p$'s leaf to the root. Because $op_1$'s \nf{Append} is finished before $op_2$'s \nf{Append} starts. So there is some block $b^\prime$  before $b$ in $n$ containing $op_1$. $op_1$ existing in $b$ an $b^\prime$ contradicts with \ref{append}.
\end{proof}
