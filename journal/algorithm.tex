...
\Function{void}{Propagate}{\typ{Node} \var{v}} \Comment{propagate blocks from \var{v}'s children to root}
    \If{{\keywordfont not} \Call{Refresh}{\var{v}}} \label{firstRefresh}  \hfill \Comment{double refresh:  \rev{if first fails, try again}}
...
Since the \enqueue\ is linearized before the \dequeue, $b_e\leq b$.  To find the left end of the range \rev{to search for $b_e$, \op{FindResponse} does} a doubling search \cite{BY76}, comparing \var{e} to the \fld{sum\sub{enq}} fields at indices $b-1, b-2, b-4, b-8, \ldots$.
