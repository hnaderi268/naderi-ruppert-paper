...
\Function{element}{FindResponse}{\typ{int} \var{b}, \typ{int} \var{i}}
    ...
    \Else \ \linecomment{response is the \var{e}th enqueue in the root}
		...
		\State find min $\var{b}'\leq \var{b}$ with $\var{root}.\fld{blocks}[\var{b}'].\fld{sum\sub{enq}} \geq \var{e}$\label{FRb}
		\State \linecomment{find rank of enqueue within its block}
		\State $\var{i}' \assign\ \var{e} - \var{root}.\fld{blocks}[\var{b}'-1].\fld{sum\sub{enq}}$\label{FRi}
