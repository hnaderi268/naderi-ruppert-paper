...
A block $b$ in a leaf represents an \nf{Enqueue()} if $b\nf{.element}\neq\nf{null}$ otherwise, if $b\nf{.element}=\nf{null}$, $b$ represents a \nf{Dequeue()}. The set of operations of block \nf{b} is the union set of the operations in leaf subblocks of $b$. We denote the set of operations of block $b$ by ${ops(b)}$ and the union set of operations of set of blocks $B$ by $ops(B)$.  We also say ${b}$ contains ${op}$ if $\nf{op}\in\nf{ops(b)}$.
\end{definition}
...
\begin{definition}
$n\nf{.blocks[}i\nf{]}$ is \emph{established} at time $t$ if $n\nf{.head}>i$. An operation is \it{established} if it is in an established block. $EST^n_t$ is the set of established operations in node $n$ at time $t$.
\end{definition}
...
Now we define the ordering of operations stored in each node. In the non-root nodes we only need ordering of operations of a type among themselves. Processes are numbered from 1 to $p$ and leaves of the tree are assigned from left to right. We will show in Lemma \ref{blockSize} that there is at most one operation from each process in a given block.
...
We show $EST_{n\nf{.left}}^{t^R}= ops(n\nf{.left.blocks[0..n.left.head$_{309}-1$]}) \subseteq ops(n.blocks_{\ref{cas}}) = ops(n\nf{.blocks[0..}n.head_{\ref{cas}}\nf{]})$.
...
Line \ref{cas} stores a block \nf{new} in $n$ that has $\nf{end\sub{left}}=n.left.head_{\ref{lastLine}}-1$. Therefore by Definition \ref{def::subblock}, after the successful \nf{CAS} in Line \ref{cas} we know all blocks in \nf{n.left.blocks[1..$n.left.head_{\ref{lastLine}}-1$]} are subblocks of \nf{n.blocks[1..$n.head_{\ref{readHead}}$]}. Because of Lemma \ref{nonDecreasingHead} we have $n.left.head_{309}-1<n.left.head_{\ref{lastLine}}-1$ and $n.head_{\ref{readHead}}<n.head_{\ref{cas}}$. From Observation \ref{lem::blocksOrder} the claim follows.  The proof for the right child is the same.
\end{proof}
\begin{figure}[hbt]
...
If $\nf{$n$.blocks[$b$].num\sub{enq}} \geq i$ then \nf{$n$.GetENQ($b$, $i$)} returns $E_i(n,b)$\nf{.element}.
\end{lemma}
\begin{proof}
We are going to prove this lemma by induction on the height of node \nf{n}. For the base case, $n$ is a leaf. Leaf blocks each contain exactly one operation, so only \nf{n.GetENQ(b,1)} can be called when $n$ is a leaf. Line \ref{getBaseCase} of \nf{$n$.GetENQ($b$, $1$)} returns the \nf{element} of the \nf{enqueue} operation stored in the $b$th block of leaf \nf{n} .

For the induction step we prove if \nf{$n$.child.GetENQ($sb$, i)} returns $E_i(n.child, sb)$ then \nf{$n$.GetENQ($b$, $i$)} returns $E_i(n,b)$.  From Definition  \ref{ordering} of $E(n,b)$ operations from the left subblocks come before the operations from the right subblocks in a block (See Figure \ref{figGet}). \nf{num\sub{enq-left}} field  in \nf{$n$.blocks[$b$]} stores the number of \nf{Enqueue()} operations from the blocks's subblocks in the left child of \nf{n}. So the $i$th enqueue operation is propagated from the right child if and only if $i$ is greater than \nf{$b$.num\sub{enq-left}}. Line \ref{leftOrRight} decides whether the \nf{i}th enqueue in  \nf{b}th block  of internal node \nf{n} is in the  left child or right child subblocks of \nf{n.blocks[b]}. By definition \ref{def::ops} to find an operation in subblocks of \nf{$n$.blocks[$i$]} we need to search in the range \texttt{$n$.left.blocks[$n$.blocks[$i$-1].end\textsubscript{left}+1..$n$.blocks[$i$].end\textsubscript{left}]} $\cup$ \texttt{$n$.right.blocks[$n$.blocks[$i$-1].end\textsubscript{right}+1..$n$.blocks[$i$].end\textsubscript{right}]}.
 
 There are $eb=n.blocks[b-1].sum_{enq-left}$ enqueues in the blocks before the left subblocks of $n$\nf{.blocks[$b$]}, so $E_i(n,b)$ is $E_{i+eb}(n.left)$ which is $E_{i^\prime}(n.left,b^\prime)$ for some $b^\prime$ and $i^\prime$. We can compute $b^\prime$ and then search for $i+eb$th enqueue in \tt{n.left}, where $i^\prime$ is \tt{i+eb-n.left.blocks[}$b^\prime-1$\tt{].sum\sub{enq}}. The parameters in Line \ref{leftChildGet} are for searching $E_{i+eb}(n.left)$ in \nf{n.left.block} in the expected range of blocks, so this \nf{BinarySearch} returns the index of the subblock containing $E_i(n,b)$.

Otherwise the enqueue we are looking for is in the right child. Because left \nf{Enqueue}s are before the right \nf{Enqueues}, there are \nf{n.blocks[b].num\sub{enq-left}} enqueues ahead from the left child. So we need to search for \nf{i-n.blocks[b].num\sub{enq-left}+ n.blocks[b-1].sum\sub{enq-right}} (Line \ref{rightChildGet}). Other parameters for the left child are chosen similar to the right child. 
...
%
%\end{proof}
Now using Corollary \ref{superRelation} on each step of the \nf{IndexDEQ} we prove its correctness.

\begin{lemma}[\nf{IndexDEQ} correctness]
 If $1\leq i \leq\nf{$n$.blocks[$b$].num\sub{deq}}$ then \nf{$n$.IndexDEQ($b$,$i$)} returns the rank in $D(root)$ of $D_i(n,b)$.
\end{lemma}
\begin{proof}
...
For the non-root nodes \nf{$n$.IndexDEQ($b$, $i$)} computes the superblock of the $b$th block of $n$ in Line \ref{computeSuper} by Corollary \ref{superRelation}. So preconditions of the invocation of \nf{BinarySearch} is satisfied. After that the order of $D_i(n,b)$ in $D(n.parent, superblock)$ is computed in Lines \ref{computeISuperStart}--\ref{computeISuperEnd}. Note that by Lemma \ref{blockSize} in each block there is at most one \nf{Dequeue} from each process and by Definition \ref{ordering} \nf{Dequeue}s in a block are ordered based on the order of its subblocks  from left to right. Then if $D_i(n,b)$ was propagated from the right child, the number of dequeues in the subblocks from the left child is considered to be ahead of the computed index (Line \ref{considerRight}). Finally \nf{IndexDEQ} is called on \nf{$n$.parent} recursively and it returns the response from induction hypothesis. 
\end{proof}

\pagebreak
We have explained the picture of proof in the first paragraph of proof section. Should I repeat here?

\begin{lemma}[satisfiability] \label{linearSat}
$L$ can be a linearization ordering.
\end{lemma}
\begin{proof}
A linearization for an execution is legal if, $op_{1}$ terminates before $op_{2}$ starts in execution then $op_{1}$ is before $op_{2}$ in the linearization. If $op_{1}$ terminates before $op_{2}$ starts,$op_{1}$\nf{.Append()} has terminated before $op_{2}$\nf{.Append()} started. From Lemma \ref{append} $op_{1}$ is in \nf{root.blocks} before $op_{2}$ starts to propagate. By definition of $L$, $op_{1}$ is linearized before $op_{2}$.
\end{proof}
Once some operations are aggregated in one block they will get propagated up to the root together and they can linearized in any order among themselves. The way we have chosen to order the operations in a block makes \nf{GetENQ}, \nf{FindResponse} efficent.

\begin{definition}\label{defNullDeq}
If a \nf{Dequeue} operation returns null it is called \it{null dequeue}, otherwise it is called \it{non-null dequeue}.
\end{definition}
...
\begin{definition}
   Assume the operations in \nf{root.blocks} are applied on an empty queue in order of $L$.  If element of \nf{Enqueue} $e$ is the response to \nf{Dequeue} $d$, we say ${Resp(d)=e}$, otherwise if ${d}$ is null dequeue then ${Resp(d)=\nf{null}}$. 
\end{definition}
...
In the next Lemma we talk about what the \nf{size} field in \nf{root} blocks store.
\begin{lemma}\label{sizeCorrectness}
  \nf{root.blocks[$b$].\size}is the \size of the queue if the operations in \nf{root.blocks[$0\cdots b$]} are applied in order of~$L$.  
\end{lemma}
\begin{proof}


We prove the claim by induction on $b$. Base case ${b=0}$ is trivial since the queue is initially empty and $\nf{root.blocks[}0\nf{].size}=0$. We are going to show the correctness of case $b=i$ assuming the correctness of $b=i-1$. By \ref{enqBeforeDeq} If there are more than $\nf{root.blocks[}i-1\nf{].size}+\nf{root.blocks[}i\nf{].sum\sub{enq}}$ dequeue operations in \nf{root.blocks[$i$]} then the queue would become empty after \nf{root.blocks[$i$]}, otherwise size of the queue after $b$th block in root is $\nf{root.blocks[}b-1\nf{].size}+ \nf{root.blocks[}b\nf{].sum\sub{enq}}- \nf{root.blocks[}b\nf{].sum\sub{deq}}$  which is same as the assignment on Line \ref{computeLength}.
\end{proof}

Next Lemma is about the relation between \#enqueues, \#non-null dequeues and size.
...
\end{lemma}
\begin{proof}
From \ref{numNullDeqBlock}, \ref{defNullDeq} and \ref{enqBeforeDeq}.
\end{proof}
...
\end{lemma}
\begin{proof}
Note that by Definition \ref{ordering} the linearization ordering  of operations will not change as new operations come so instead of talking about the linearization of operations before the $E_i(root, b)$ we talk about what if the whole operation in the linearization are applied on a queue.

$D_i(root,b)$ is $D_{root.blocks[b-1].sum_{deq}+i}(root)$ from the Definitions \ref{ordering} and \it{$sum_{deq}$}.  $D_i(root,b)$ returns \nf{null} if $\nf{root.blocks[}$b-1$\nf{].size}+ \nf{root.blocks[}b\nf{].num\sub{enq}}- i <0$ by Lemma \ref{nullReturn} (Line \ref{checkEmpty}). Otherwise if it is $k$th non-null dequeue in $L$ it returns $k$th enqueue by Observation \ref{responseToADeq}. By Lemma \ref{numberOfNND} there are \nf{root.blocks[$b-1$].sum\sub{enq} $-$ root.blocks[$b-1$].\size} non-null dequeue operations in $\nf{root.blocks[}0\cdots b-1\nf{]}$. Note that the dequeues in \nf{root.blocks[$b$]} before $D_i(root,b)$ are non-null dequeues. So the response is $E_{i - root.blocks[b-1].size + root.blocks[b-1].sum\sub{deq}}(root)$ (Line \ref{computeE}). See figure \ref{computeResponseDetail}.

After computing \nf{e}, the code finds \nf{$b$,$i$} such that $E_i(root,b)=E_e(root)$ by \nf{DSearch} and then finds its \nf{element} using \nf{GetEnq} (Line \ref{findAnswer}).
\end{proof}

\begin{figure}[hbt]  
  \center\includegraphics[width=2in]{pics/computeResponseDetail.png}\caption{The position of $D_i(root,b)$.}
\label{computeResponseDetail}
\end{figure}

\begin{lemma}[correctness]\label{linearCorrect}
  If operations are applied as $L$ on a sequential queue, the sequence of the responses would be the same as our algorithm.
\end{lemma}
\begin{proof}
Enqueue operations do not return any value so they are fine. Response of a dequeue in sequential execution of $L$ is same as our algorithm from \ref{computeHead}.  
\end{proof}
