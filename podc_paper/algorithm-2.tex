...
\HERE{Idea:  replace last array with a bit in blocks that gets set when somebody dequeues an element in the block.  then can use the max index of any marked block minus 1 as the last completed block.  
In order to ensure that blocks of dequeues are also cleared out,
maybe dequeues that return null have to mark all blocks along the dequeue's path?
}
...
\here{Walk reader through the code (and how it is different from basic versino)}

\op{MaxBlock} is a query operation on a RBT that returns the block with the maximum \fld{index}.
We could keep a pointer to the maximum block in the root of the RBT so that \op{MaxBlock}
can be done in constant time.

\here{Emphasize that RBT nodes have \emph{pointers to} blocks.  Thus when RBT does node copying,
it copies the pointer to the block; it does not make a copy of the block itself.
This is important because if we mark a block that has been node-copied, we want the block to still be marked.  Similarly for super field.}
...
\Function{void}{Enqueue}{\typ{Object} \var{e}} 
    \gc{
    \State \var{h} \assign \Call{MaxBlock}{\var{leaf}.\fld{blocks}}.\fld{index}
...
\Function{Object}{Dequeue()}{} 
    ...
    \State \Call{Append}{\var{B}}
    \State $\langle \var{b}, \var{i}\rangle \assign\ \Call{IndexDequeue}{\var{leaf}, \gc{h+1}, 1, \fa}$
    \gc{
    \State $\var{res} \assign \Call{FindResponse}{b, i}$
	\If{$\var{res} = \nl$} \linecomment{mark blocks}
		\State $\Call{IndexDequeue}{\var{leaf}, h+1, 1, \tr}$
	\EndIf
...
\Function{void}{Append}{\typ{Block} \var{B}} 
    ...
    \gc{\State \var{leaf}.\fld{blocks} \assign \Call{AddBlock}{\var{leaf}.\fld{blocks}, \var{B}}\label{appendLeafGC}}
...
\gc{
\Function{RBT}{AddBlock}{\typ{RBT} \var{T}, \typ{Block} \var{B}}
    ...
    \If{\var{B}.\fld{index} is a multiple of $p^2$}
        ...
        \State \Call{Help}{}
        \State $m$ \assign\ maximum \fld{index} of marked block in $T$
...
\Function{boolean}{Refresh}{\typ{Node} \var{v}}
    ...
    \Else  
    	\gc{
	    \State $T' \assign \Call{AddBlock}{T, \var{new}}$
...
\Function{$\langle\typ{int}, \typ{int}\rangle$}{IndexDequeue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}, \gc{\typ{boolean} \var{mark}}}
    ...
    \gc{
    \If{\var{mark}} \var{v}.\fld{blocks}[\var{b}].\fld{mark} \assign \tr
    \EndIf
    }
...
\Function{element}{GetEnqueue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}} \Comment{returns argument of \var{i}th enqueue in $E(\var{v}.\fld{blocks}[\var{b}])$}
	...
	\gc{\State $\var{v}.\fld{blocks}[\var{b}].\fld{marked} := \tr$}
