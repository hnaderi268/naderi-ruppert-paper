...
compare-and-swap (CAS) instruction\footnote{\rev{A CAS(\var{X}, \var{old}, \var{new}) instruction updates a variable \var{X} to the value \var{new} and returns \tr\ if \var{X}'s value is \var{old}; otherwise the CAS leaves \var{X} unchanged and returns \fa.}}.
