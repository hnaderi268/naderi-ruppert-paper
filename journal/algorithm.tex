...
\rev{Using terminology from \cite{DBLP:conf/fsttcs/JayantiP05}, we call the procedure that does} this three-step sequence a
...
\rev{Thus, after performing a double \op{Refresh},} $P$ can continue 
...
If a block contained an explicit \rev{listing} of its sequences of \enqueues\ and \dequeues,
it would take $\Omega(p)$ \rev{steps} to construct a block, which would be too slow for task \ref{construct}.
...
Each block in an internal \node\ $\var{v}$ has fields \eleft\ and \eright\ that store the indices of the block's last direct subblock in $\var{v}$'s left and right child, \rev{respectively}.  
...
that store the total numbers of \enqueues\ and \dequeues, \rev{respectively}, in $\var{v}.\fld{blocks}[1..\var{b}]$.
...
\rev{Recall that $p$ is the number of processes and $q$ is the number of elements in the queue.}
We shall show \rev{in Section \ref{sec-analysis}} that the binary search in the root can be done in $O(\log p + \log q)$ steps,
...
we can easily see how to construct a new block $B$ during a \op{Refresh} in $O(1)$ \rev{steps}.
...
We shall show that this happens soon enough \rev{to ensure} that $B.\fld{super}$ can differ from the true index of \rev{$B$'s superblock}
by at most 1.
...
\Function{void}{Enqueue}{\typ{Object} \var{e}} 
    ...
    \State \Call{Append}{\var{B}}\Comment{\rev{append \op{Enqueue} to \var{leaf.}\fld{blocks} and propagate to root}}\label{enqAppend}
\EndFunction{Enqueue}
...
\Function{Object}{Dequeue}{} 
    ...
    \State \Call{Append}{\var{B}}\Comment{\rev{append \dequeue\ to \var{leaf.}\fld{blocks} and propagate to root}}\label{deqAppend}
    \State $\langle \var{b}, \var{i}\rangle$ \assign\ \Call{IndexDequeue}{\var{leaf}, $\var{leaf.}\head-1$, $1$} \Comment{\rev{find where \op{Deqeueue} is in \var{root.}\fld{blocks}}}\label{invokeIndexDequeue}
    \State \Return{\Call{FindResponse}{\var{b, i}}}\Comment{\rev{find response \dequeue\ should return}}\label{deqRest}
\EndFunction{Dequeue}
...
\Function{void}{Propagate}{\typ{Node} \var{v}} \Comment{propagate blocks from \var{v}'s children to root}
    \If{{\keywordfont not} \Call{Refresh}{\var{v}}} \label{firstRefresh}  \hfill \Comment{double refresh:  \rev{if first \op{Refresh} fails, try again}}
...
\Function{boolean}{Refresh}{\typ{Node} \var{v}} \Comment{try to append a new block $B$ to \var{v}.\fld{blocks}}
    ...
    \ForEach{\fld{dir} {\keywordfont{in}} $\{\fld{left, right}\}$} \Comment{\rev{help advance $v$'s children's \var{head} indices, if necessary}}\label{startHelpChild1}
    ...
    \State \var{B} \assign\ \Call{CreateBlock}{\var{v, h}}\Comment{\rev{create new block to append to \var{v}.\fld{blocks}}}\label{invokeCreateBlock}
    \If{$\var{B} = \nl$} \Return{\tr}\Comment{\rev{if no new operations to propagate to $v$, do nothing}}\label{addOP} 
	\Else
	    \State \var{result} \assign\ \Call{CAS}{\var{v}.\fld{blocks}[\var{h}], \nl, \var{B}}\Comment{\rev{try to add $B$ to \var{v}.\fld{blocks}}}\label{cas}
    	\State \Call{Advance}{\var{v, h}} \Comment{\rev{advance $v.head$ from $h$ to $h+1$}}\label{advance}
    	\State \Return{ \var{result}}\Comment{\rev{return true iff \op{CAS} succeeded}}
	\EndIf
\EndFunction{Refresh}
\end{algorithmic}
...
\Function{$\langle\typ{int}, \typ{int}\rangle$}{IndexDequeue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}} 
    ...
    \Else \Comment{first, find the superblock of $v.\fld{blocks}[b]$}
    	...
    	\State \var{sup} \assign\ \var{v}.\fld{blocks}[\var{b}].\fld{super}\Comment{\rev{actual index of superblock is either \var{sup} or $\var{sup}+1$}}\label{idsup1}
	    ...
	    \State \linecomment{\rev{add \# \dequeues\ in blocks of $v$ before $v.\fld{blocks}[b]$ within superblock $v.\fld{parent.blocks}[\var{sup}]$}}
        ...
        \If{$\fld{dir} = \fld{right}$} \Comment{\rev{add \# \dequeues\ in superblock from left sibling}}
        ...
        \State \Return\Call{IndexDequeue}{\var{v}.\fld{parent}, \var{sup}, \var{i}} \Comment{\rev{recurse up the tree}}
    \EndIf
...
\Function{element}{FindResponse}{\typ{int} \var{b}, \typ{int} \var{i}} \com\ find response to \var{i}th \dequeue\ in $D(\var{root}.\fld{blocks}[\var{b}])$
    ...
    \Else \ \Comment{response is the \var{e}th \enqueue\ in the root}
		...
		\State use \rev{doubling} search to find min $b_e\leq \var{b}$ with $\var{root}.\fld{blocks}[b_e].\fld{sum\sub{enq}} \geq \var{e}$\label{FRb}
		\State $i_e \assign\ \var{e} - \var{root}.\fld{blocks}[b_e-1].\fld{sum\sub{enq}}$\Comment{find rank of \enqueue\ within its block}\label{FRi}
        \State \Return \Call{GetEnqueue}{\var{root}, $b_e$, $i_e$}\Comment{\rev{find argument of that \enqueue}}\label{findAnswer}
    \EndIf
\EndFunction{FindResponse}
\end{algorithmic}
\caption{Queue implementation's helper routines. \rev{Recall that $D(B)$ is the sequence of \dequeues\ represented by block $B$.}\label{code2}}
\end{figure*}
...
\end{algorithmic}
\caption{Queue implementation's \op{GetEnqueue} routine.  \rev{Recall that $E(B)$ is the sequence of \enqueues\ represented by block $B$}.\label{code3}}
\end{figure*}

An \opemph{Enqueue}(\var{e}) 
\rev{creates a new block at line \ref{enqNew} and sets its \fld{sum} fields to indicate
...
\rev{creates a new block at line \ref{deqNew} and sets its \fld{sum} fields to indicate
...
\rev{Lines \ref{startHelpChild1}--\ref{endHelpChild1} ensure that the \fld{head} values
...
\rev{If the new block contains no operations, \op{CreateBlock} returns \nl\ at line \ref{testEmpty} to indicate
 there is no need to install~it.
Otherwise,} if \var{B}
is going to be installed in the root, line \ref{computeLength} \rev{uses Equation (\ref{size-eqn}) to compute $B$'s} \fld{size} field, which
...
\rev{The routine next searches for}
the index $b_e$ of the block in \var{root}.\fld{blocks} that contains 
the \var{e}th \enqueue.
Since the \enqueue\ is linearized before the \dequeue, \linebreak $b_e\leq b$.  To find the left end of the range \rev{to search for $b_e$, \op{FindResponse} does} a doubling search \cite{BY76}, comparing \var{e} to the \fld{sum\sub{enq}} fields at indices $b-1, b-2, b-4, b-8, \ldots$.
\rev{Then, the doubling search performs a binary search on the \fld{sum\sub{enq}} fields of \var{root}.\fld{blocks} within that range to find $b_e$.}
Finally, the call to \op{GetEnqueue} \rev{at line \ref{findAnswer}} traces down through the tree to find the required \enqueue\ in a leaf.
...
\rev{The test at line \ref{leftOrRight} determines which child of \var{v} contains
...
field \rev{of the child's \var{blocks} array.
