...
\Function{Object}{Dequeue()}{} 
    ...
    \State \Call{Append}{\var{n}}
    \State \var{<b, i>} \assign\ \Call{IndexDequeue}{\var{leaf}, \var{leaf}.\head, 1}
...
\Function{Block}{CreateBlock}{\typ{Node} \var{v}, \typ{int} \var{i}} 
    ...
    \State\linecomment{(return \nl\ if no blocks need to be propagated to \var{v})}
    \State let \var{new} be a new \typ{Block} \label{initNewBlock}
    \State \var{new}.\eleft \assign\ $\var{v}.\fld{left}.\head - 1$
...
\Function{<int, int>}{IndexDequeue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}}
    \State \linecomment{return \var{<x, y>} such that \var{i}th dequeue in $D(\var{v}.\fld{blocks}[\var{b}])$}
    ...
    \Else
        ...
        \If{$\fld{dir} = \fld{right}$} 
        	\State \hangbox{\var{i} += $\var{v}.\fld{blocks}[\var{v}.\fld{parent.blocks}[\var{sup}].\eleft].\fld{sum\sub{deq}} -$\\
...
\Function{element}{FindResponse}{\typ{int} \var{b}, \typ{int} \var{i}}
    ...
    \State \hangbox{\var{num\sub{enq}} \assign\ $\var{root}.\fld{blocks}[\var{b}].\fld{sum\sub{enq}} - $\\
...
\Function{element}{GetEnqueue}{\typ{Node} \var{v}, \typ{int} \var{b}, \typ{int} \var{i}} \Comment{returns argument of \var{i}th enqueue in $E(\var{v}.\fld{blocks}[\var{b}])$}
    ...
    \Else 
        ...
        \State find minimum $\var{b}'$ in range [\var{v}.\fld{blocks}[\var{b}-1].\edir+1..\var{v}.\fld{blocks}[\var{b}].\edir] s.t. $\var{v}.\fld{dir.blocks}[\var{b}'].\fld{sum\sub{enq}} \geq \var{i} + \var{prev\sub{dir}}$
...
(The need for helping on line \ref{helpAdvance} is explained below, when we discuss the \op{IndexDequeue} routine.)
...
within that block  of the \var{i}th \op{Dequeue} of the block stored in \var{v}.\fld{blocks}[\var{b}].
Let $R\sub{\var{v}}$ be the successful \nf{Refresh} on node \var{v}
whose CAS wrote a block $B$ into \var{v}.\fld{blocks}[\var{b}].
\here{---}
