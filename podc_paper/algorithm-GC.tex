...
the new block's \fld{index} is a multiple of the constant $G$, which we choose to be $(p\ceil{\log p})^2$.
...
\end{minipage}
\vspace*{-4mm}
\caption{Bounded-space queue implementation.\label{pseudocode1GC}  $G$ is a constant, which we choose to be $(p \ceil{\log p})^2$.}
\end{figure}
