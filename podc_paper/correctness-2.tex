...
Then some other \op{CAS} on $v.\fld{blocks}$ succeeds between the time each \op{Refresh}
reads $v.\fld{blocks}$ at line \ref{refreshReadTGC} and performs its \op{CAS} at line \ref{casGC}.
Consider the \op{Refresh} $R_3$ that does this successful \op{CAS} during $R_2$.
$R_3$ must have read $v.\fld{blocks}$ after the successful \op{CAS} during $R_1$.
