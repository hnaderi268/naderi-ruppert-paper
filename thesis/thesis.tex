...
\end{subfigure}
\hspace{6em}
\caption{\label{fig::propagstep} Before and after of a \sc{Refresh}$(n)$ with successful \nf{CAS}. Operations propagating from the left child are numbered with $l$ and from the right child by $r$.}
\end{center}
\end{figure}
...
\paragraph{Introduce CAS} We do not intend to get into our implementation but just for information we use \nf{CAS} (Compare \& Swap) operations to implement the Try to Append mechanism told in the previous paragraph. After a process appended its operation into its leaf it can call \nf{Refresh} on the path up to root two times on each node. So with poly-logarithmic \nf{CAS}es we can have a tree agreeing on the linearization. This solution makes us overcome the \nf{CAS} Retry Problem.
