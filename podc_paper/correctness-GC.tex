...
\here{Should add some discussion about the fact that we keep the newest finished block, in case some operation contained in a block with index i also needs to look at fields of the block with index i-1}
...
It follows from \Cref{RBTbound} and the fact that $G=(p\ceil{\log p})^2$
